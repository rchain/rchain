...
class RadixTreeSpec3 extends FlatSpec {

  def sToBv(str: String) = ByteVector.fromHex(str).get

  implicit class TreeBinOps(rootNode: Vector[Child]) {
    def binUpdate(radixKey: String, radixValue: ByteVector)(impl: RadixTreeImpl): Vector[Child] =
      impl.update(rootNode, sToBv(radixKey), radixValue)

    def binDelete(radixKey: String)(impl: RadixTreeImpl): Vector[Child] =
