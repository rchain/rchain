...
/**
  ...
  */
final case class TransactionAPIImpl[F[_]: Sync: Concurrent](
      ...
      ts <- report match {
             case Right(b) =>
               for {
                 ...
                 systemDeployTransactions = b.systemDeploys.flatMap(
                   s => {
            ...
            p =>
              p.report.value match {
                case ReportProduceProto(channel, _) =>
                  channel.flatMap(c => Some(c == transaction.retUnforgeable)).getOrElse(false)
                case _ => false
              }
          )
          .flatMap(
