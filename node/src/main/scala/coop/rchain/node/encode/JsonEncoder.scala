...
object JsonEncoder {
  ...
  implicit val decodeBlake2b512Random: Decoder[Blake2b512Random] =
  ...
  def convertCcodecToScodec[A](encoder: Encoder[A], decoder: Decoder[A]): SCodec[A] =
    new SCodec[A] {
      override def encode(value: A): Attempt[BitVector] = utf8.encode(encoder.apply(value).noSpaces)
            ...
            s =>
              Attempt
                .fromEither(
                  parse(s.value)
                    .leftMap(
                      e => Err.General(e.message, e.underlying.getStackTrace.map(_.toString).toList)
                    )
                    .map(j => decoder.decodeJson(j))
                )
                .flatMap(
                  r =>
                    Attempt.fromEither(
                      r.leftMap(d => Err.General(d.message, List.empty))
