...
import coop.rchain.node.casper.CasperNodeState._
...
import coop.rchain.v2.validation.Offence.SlashableOffence
...
case class CasperNodeState[M: Ordering, U, S](
   ...
   *
   * All messages received by the node that belong to the network and have valid signature should be piped into this call.
...
object CasperNodeState {
   ...
   * Todo latest messages are very important, think through carefully
   */
  private def updateLatestMessages[M, S](latestMessages: LatestMessages[M, S], newMessage: M)(
      seqNum: M => Long,
      sender: M => S
