...
class Key(keyIn: String) extends TermTree with Ordered[Key] {
  ...
  def createParseTree(
      lexer: KeyLexer): collection.mutable.ArrayBuffer[TermTree] = {
    var keyName = ""
    var paramsArray = new collection.mutable.ArrayBuffer[TermTree]()
  ...
  def unifyQuery(keyValueStore: KeyValueStore)
    : collection.mutable.LinkedHashSet[Array[Binding]] = {
    var bindings = collection.mutable.LinkedHashSet[Array[Binding]]()
  ...
  def compare(key: Key): Int = {
    ...
    if (term.length > key.term.length) { return 1 } else { return -1 }
  }
