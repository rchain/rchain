...
object QueryTools {
  ...
  def unifyParams(queryParams: collection.mutable.ArrayBuffer[TermTree],
                  keyParams: collection.mutable.ArrayBuffer[TermTree])
    : (Boolean, Array[Binding]) = {
    if (queryParams.length != keyParams.length) {
      return (false, Array[Binding]())
    }

    var bindings = new collection.mutable.ArrayBuffer[Binding]()
  ...
  def createParamsSubstition(params: Params,
    ...
    var returnParams = new collection.mutable.ArrayBuffer[TermTree]
  ...
  def queryResultsToArrayString(
      query: Key,
      queryResult: collection.mutable.LinkedHashSet[Array[Binding]],
      store: KeyValueStore): Unification = {
    var standardRep = new collection.mutable.ArrayBuffer[String]()
  ...
  def DivideUnificationResults(uniResults: Array[Binding]): UniRep = {
    val varInQuery = new collection.mutable.ArrayBuffer[Binding]()
    val varInKey = new collection.mutable.ArrayBuffer[Binding]()
