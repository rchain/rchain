...
int
readError (FILE* f, const char* fmt, ...)
{
    va_list args;
    int status;
...
char
getSafeChar (FILE* f, char escapeChar)
{
    char c;
...
int
putSafeChar (char c, FILE* f, char escapeChar)
{
    if (c == escapeChar) {
	putc(c, f);
	putc(c, f);
...
int
safeStrlen (char* str, char escapeChar)
{
    int n = 0;
...
char*
safeStrcpy (char* dest, char* src, char escapeChar)
{
