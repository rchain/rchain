...
Tuple::Tuple (int size, Tuple* master, int offset, int n, Ob* init)
    : Ob(sizeof(Tuple) + size*sizeof(Ob*), CLASS_META(Tuple), CLASS_SBO(Tuple))
{
    Ob** p = &elem(0);
    Ob** q = &master->elem(offset);
    int i = n;
    for (; i--;) *p++ = *q++;
    for (i = size-n; i--;) *p++ = init;
    Tuple::updateCnt();
}
...
Tuple::Tuple (Tuple* t1, Tuple* t2)
    : Ob(sizeof(Tuple)+(t1->numberOfElements()+t2->numberOfElements())*sizeof(Ob*),
...
Tuple::Tuple (Tuple* t, int n, Tuple* rest)
    : Ob(sizeof(Tuple)+(n+1)*sizeof(Ob*), CLASS_META(Tuple), CLASS_SBO(Tuple))
{
    Ob** newp = &elem(0);
    Ob** oldp = &t->elem(0);
    for (short int i = n; i--;)
...
Tuple*
Tuple::create (int size, Tuple* master, int offset, int n, Ob* init)
{
    if (size == 0)
	return NIL;
...
Tuple*
Tuple::create (int size, int offset, Tuple* rest)
{
    if (size == 0)
	return NIL;
...
Tuple*
Tuple::create (Tuple* t1, Tuple* t2)
{
  if (t1 == NIL)
    return t2;
...
StdExtension*
Tuple::becomeExtension (Ob* new_meta, Ob* new_parent)
{
...
Tuple*
Tuple::makeSlice (int offset, int howmany_)
{
    return (this == NIL
	    ? NIL
...
Tuple*
Tuple::makeTail (int entriesToSkip)
{
...
Ob*
Tuple::cloneTo (Ob* new_meta, Ob* new_parent)
{
    return (this == NIL
	    ? NIL
...
Tuple*
consstar (Tuple* prefix, int prefixsize, Tuple* suffix)
{
    PROTECT(prefix); PROTECT(suffix);
    if( prefixsize == 0 )
	return suffix;
...
DEF("null?",obNullQ, 1, 1)
{
    return (ARG(0) == NIL ? RBLTRUE : RBLFALSE);
}
