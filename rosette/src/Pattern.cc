...
inline
Pattern::Pattern (int sz, Ob* meta, Ob* parent)
    : Ob(sz, meta, parent)
{ }
...
Ob*
Pattern::cloneTo (Ob* new_meta, Ob* new_parent)
{
...
BUILTIN_CLASS(IdPattern)
{
    OB_FIELD("symbol",	IdPattern, symbol);
}
...
void
IdPattern::stuffKeys (Tuple* keys, int offset)
{
...
bool
IdPattern::matchIntoArgvec (Tuple* argvec, int offset, Ob* val, int)
{
...
BUILTIN_CLASS(ConstPattern)
{
    OB_FIELD("val",	ConstPattern, val);
}


ConstPattern::ConstPattern (Ob* val)
    : Pattern(sizeof(ConstPattern), CLASS_META(ConstPattern), CLASS_SBO(ConstPattern)),
      val(val)
{
...
bool
ConstPattern::matchIntoArgvec (Tuple* argvec, int offset, Ob* val, int)
{
    if (val == this->val) {
	ASSIGN(argvec, elem(offset), val);
	return TRUE;
...
int
ConstPattern::numberOfKeys (void)
{
    return 0;
}
...
void
ConstPattern::stuffKeys (Tuple*, int)
{ }



inline CompoundPattern::CompoundPattern (int sz, Ob* meta, Ob* parent, TupleExpr* e)
    : Pattern(sz, meta, parent),
      expr(e)
{ }


int
CompoundPattern::keyExtent (int)
{
    NI("keyExtent");
    return 0;
}
...
Tuple*
CompoundPattern::match (Tuple*, int)
{
    NI("match");
...
BUILTIN_CLASS(IdVecPattern)
{
    OB_FIELD("expr",	IdVecPattern, expr);
}


IdVecPattern::IdVecPattern (TupleExpr* te)
    : CompoundPattern(sizeof(IdVecPattern), CLASS_META(IdVecPattern), CLASS_SBO(IdVecPattern), te)
{
...
IdVecPattern*
IdVecPattern::create (TupleExpr* te)
{
...
int
IdVecPattern::numberOfKeys (void)
{
    return expr->numberOfElements();
}


void
IdVecPattern::stuffKeys (Tuple* keys, int offset)
{
...
int
IdVecPattern::keyExtent (int componentNum)
{
    return componentNum;
}
...
Tuple*
IdVecPattern::match (Tuple* argvec, int nargs)
{
...
bool
IdVecPattern::matchIntoArgvec (Tuple* argvec, int offset, Ob* val, int nargs)
{
    int need = expr->numberOfElements();
...
IdAmperRestPattern*
IdAmperRestPattern::create (TupleExpr* te)
{
...
void
IdAmperRestPattern::stuffKeys (Tuple* keys, int offset)
{
    int n = expr->numberOfElements();
...
Tuple*
IdAmperRestPattern::match (Tuple* argvec, int nargs)
{
    int need = expr->numberOfElements();
...
bool
IdAmperRestPattern::matchIntoArgvec (Tuple* argvec, int offset, Ob* val, int nargs)
{
    int need = expr->numberOfElements();
...
int
IdAmperRestPattern::numberOfKeys (void)
{
...
int
IdAmperRestPattern::keyExtent (int componentNum)
{
    return componentNum;
}
...
ComplexPattern::ComplexPattern (TupleExpr* te, Tuple* pv, Tuple* ov)
    : CompoundPattern(sizeof(ComplexPattern),
		      CLASS_META(ComplexPattern),
		      CLASS_SBO(ComplexPattern),
		      te),
		      patvec(pv),
		      offsetvec(ov)
{
...
ComplexPattern*
ComplexPattern::create (TupleExpr* te)
{
...
int
ComplexPattern::numberOfKeys (void)
{
    return ComplexPattern::keyExtent(patvec->numberOfElements()) + IS_SYM(expr->rest);
}


void
ComplexPattern::stuffKeys (Tuple* keys, int offset)
{
    int n = patvec->numberOfElements();
    if (expr->rest != NILexpr)
	ASSIGN(keys, elem(offset+FIXVAL(offsetvec->elem(n))), expr->rest);
    while (n--)
	((Pattern*)patvec->elem(n))->stuffKeys(keys, offset+FIXVAL(offsetvec->elem(n)));
}


int
ComplexPattern::keyExtent (int componentNum)
{
...
Tuple*
ComplexPattern::match (Tuple* argvec, int nargs)
{
    // This code assumes that nargs > 0, so that "Tuple::create"
    // doesn't need to be guarded for NIL.

    PROTECT_THIS(ComplexPattern); PROTECT(argvec);
...
bool
ComplexPattern::matchIntoArgvec (Tuple* argvec, int offset, Ob* val, int nargs)
{
    ...
    bool matched = TRUE;
    PROTECT_THIS(ComplexPattern); PROTECT(argvec); PROTECT(valvec);

    if (expr->rest == NILexpr && nargs == need) {
	while (need-- && matched) {
	    Pattern* pat = (Pattern*) SELF->patvec->elem(need);
	    matched = pat->matchIntoArgvec(argvec,
					   offset + SELF->keyExtent(need),
					   valvec->elem(need));
	}
    }
...
Template*
Template::create (Tuple* keys, Ob* keymeta, CompoundPattern* pat)
{
    /*
...
Ob*
Template::cloneTo (Ob* new_meta, Ob* new_parent)
{
    return (this == NILtemplate ? (Ob*)this : Ob::cloneTo(new_meta, new_parent));
}



Pattern*
TupleExpr::makePattern (EMPTY)
{
    if (this == NILexpr)
	return NILpattern;
...
MODULE_INIT(Pattern)
{
    ...
    NILtemplate	= (Template*) heap->tenure(Template::create (NIL, NILmeta, NILpattern));
}
