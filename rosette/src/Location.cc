...
Word32
nontrivial_pre_fixnum_to_addr(int x)
{ Word32 y = x;
  if ( x  <  END_SMALL_ADDR ) return   y;
 if ( x >= END_SMALL_ADDR) return (y + DBEGIN - END_SMALL_ADDR) ;
  printf("error");
 return 0;
}

int
nontrivial_addr_to_pre_fixnum(Ob *x)
{if (x < (void *) END_SMALL_ADDR) return (int) x;
 if (x >= (void *) DBEGIN) return (int) ((unsigned int)x - DBEGIN +END_SMALL_ADDR);
 printf("error");
 return 0;
}
#endif
...
bool
store (Location loc, Ctxt* k, Ob* val)
{
	...
	if (GET_ARGREG_INDEX(loc)  >= k->argvec->numberOfElements())
      ...
      case LT_LexVariable:
	return BASE(k->env)->setLex(GET_LEXVAR_IND(loc),
				    GET_LEXVAR_LEVEL(loc),
	...
	return ! IS_FIXNUM(val)
	       || BASE(k->env)->setField(GET_BITFIELD_IND(loc),
					 GET_BITFIELD_LEVEL(loc),
					 GET_BITFIELD_OFFSET(loc),
					 SPANSIZE(GET_BITFIELD_SPAN(loc)),
					 (unsigned long)(FIXVAL(val)))
		 == INVALID;

      case LT_BitField00:
	return ! IS_FIXNUM(val)
	       || BASE(k->env)->setField(0,
					 0,
					 GET_BITFIELD00_OFFSET(loc),
...
fetch (Location loc, Ctxt* k)
{
	...
	return (GET_ARGREG_INDEX(loc) < k->argvec->numberOfElements()
      ...
      case LT_LexVariable:
	return BASE(k->env)->getLex(GET_LEXVAR_IND(loc),
				    GET_LEXVAR_LEVEL(loc),
	...
	return BASE(k->env)->getAddr(GET_ADDRVAR_IND(loc),
      ...
      case LT_BitField:
	return BASE(k->env)->getField(GET_BITFIELD_IND(loc),
				      GET_BITFIELD_LEVEL(loc),
				      GET_BITFIELD_OFFSET(loc),
				      SPANSIZE(GET_BITFIELD_SPAN(loc)),
...
void
printRep (Location loc, char* buf)
{
    /*
    ...
    static char* names[] = {"rslt", "trgt", "argvec", "env", "code", "ctxt",
			    "self", "self-env", "rcvr", "monitor"};

    switch (GET_GENERIC_TYPE(loc)) {

      case LT_CtxtRegister:
        if (0 <= GET_CTXTREG_INDEX(loc) && GET_CTXTREG_INDEX(loc) < NumberOfCtxtRegs)
	    strcpy(buf, names[GET_CTXTREG_INDEX(loc)]);
	else
	    sprintf(buf, "unknown ctxt register 0x%x", (int) GET_CTXTREG_INDEX(loc));
	break;
	...
	break;
      ...
      case LT_BitField:
	sprintf(buf, GET_BITFIELD_IND(loc) ? "%sfld[%d,(%d),%d]" : "%sfld[%d,%d,%d]",
		GET_BITFIELD_SIGN(loc) ? "s" : "u",
		(int)GET_BITFIELD_LEVEL(loc),
		(int)GET_BITFIELD_OFFSET(loc),
		(int)SPANSIZE(GET_BITFIELD_SPAN(loc)));
	break;

      case LT_BitField00:
	sprintf(buf, "%sfld[%d,%d]",
		GET_BITFIELD00_SIGN(loc) ? "s" : "u",
...
Ob*
valWrt (Location loc, Ob* v)
{
    switch (GET_GENERIC_TYPE(loc)) {
      case LT_LexVariable:
	return BASE(v)->getLex(GET_LEXVAR_IND(loc),
			       GET_LEXVAR_LEVEL(loc),
			       GET_LEXVAR_OFFSET(loc));
      case LT_AddrVariable:
	return BASE(v)->getAddr(GET_ADDRVAR_IND(loc),
				GET_ADDRVAR_LEVEL(loc),
				GET_ADDRVAR_OFFSET(loc));
      case LT_BitField:
	return BASE(v)->getField(GET_BITFIELD_IND(loc),
				 GET_BITFIELD_LEVEL(loc),
				 GET_BITFIELD_OFFSET(loc),
				 SPANSIZE(GET_BITFIELD_SPAN(loc)),
				 GET_BITFIELD_SIGN(loc));
      case LT_BitField00:
	return BASE(v)->getField(0,
				 0,
				 GET_BITFIELD00_OFFSET(loc),
				 SPANSIZE(GET_BITFIELD00_SPAN(loc)),
				 GET_BITFIELD00_SIGN(loc));
      case LT_GlobalVariable:
	return BASE(GlobalEnv)->getLex(TRUE,
				       0,
				       GET_GLOBALVAR_OFFSET(loc));
      case LT_Limbo:
	return ABSENT;
      default:
...
Ob*
setValWrt (Location loc, Ob* v, Ob* val)
{
    switch (GET_GENERIC_TYPE(loc)) {
      case LT_LexVariable:
	return BASE(v)->setLex(GET_LEXVAR_IND(loc),
			       GET_LEXVAR_LEVEL(loc),
			       GET_LEXVAR_OFFSET(loc),
			       val);
      case LT_AddrVariable:
	return BASE(v)->setAddr(GET_ADDRVAR_IND(loc),
				GET_ADDRVAR_LEVEL(loc),
				GET_ADDRVAR_OFFSET(loc),
			       val);
      case LT_BitField:
...
void
adjustLevel (Location loc, int adjustment)
{  unsigned int tem;
    switch (GET_GENERIC_TYPE(loc)) {
      case LT_LexVariable:
...
Location
AddrVar (int level, int offset, int indirect)
{
...
Location
BitField (int level, int offset, int span, int indirect, int sign)
{
...
Location
BitField00 (int offset, int span, int sign)
{
