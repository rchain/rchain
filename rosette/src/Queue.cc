...
Ob*
Queue::cloneTo (Ob* new_meta, Ob* new_parent)
{
    PROTECT_THIS(Queue);
...
Queue::enqueue (Ob* val)
{
	...
	PROTECT_THIS(Queue); PROTECT(val);
...
Ob*
Queue::subObject (int start, int size)
{
    PROTECT_THIS(Queue);
...
Ob*
Queue::patternDequeue (Tuple* pat)
{
  int qcapacity = elems->numberOfElements();
  int qsize = FIXVAL(nElems);
  int qh = FIXVAL(qHead);
...
MboxQueue::MboxQueue (Tuple* elems)
    : Queue(sizeof(MboxQueue), CLASS_META(MboxQueue), CLASS_SBO(MboxQueue), elems)
{
    MboxQueue::updateCnt();
}
...
Ob*
MboxQueue::maybeDequeue (Ob* enabledSet)
{
    int qcapacity = elems->numberOfElements();
    int qsize = FIXVAL(nElems);
    int qh = FIXVAL(qHead);
...
DEF("queue-read-nth",queueReadNth, 2, 2)
{
  ...
  int i = (FIXVAL(queue->qHead) + n)
          % queue->elems->numberOfElements();
