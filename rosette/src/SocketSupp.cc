...
*/
void
ConnectEventToRosette (VM_EVENT type, int fd, void* ob)
{
...
void
AcceptEventToRosette (VM_EVENT type, int fd, void* ob)
{
  PROTECT (ob);
  int more;
...
void
TcpEventToRosette (VM_EVENT type, int fd, void* ob)
{
  int r, more;
...
void
StringEventToRosette (VM_EVENT type, int fd, void* ob)
{
  int r, more;
...
int tcpListen (int port, Ob* acter)
{
  ...
  result = 1;
  setsockopt (fd, SOL_SOCKET, SO_REUSEADDR, (const char*)&result, sizeof result);
  setSocketAsync(fd);
