...
Ob*
checkFxResult (Prim* prim, Ctxt* ctxt, int answer)
{
...
DEF("fx-",fxMinus, 1, 2)
{
    ...
    switch (NARGS) {
      ...
      case 2: {
	  CHECK_FIXNUM(0, m); CHECK_FIXNUM(1, n);
	  result = m-n;
	  break;
      }
    }
...
{
  CHECK_FIXNUM(0, m);
  if (!IS_FIXNUM(ARG(1))) return RBLFALSE;
  int n = FIXVAL(ARG(1));
  return RBLBOOL(m < n);
}
...
Float::Float (Rfloat v)
    : BinaryOb(sizeof(Float), CLASS_META(Float), CLASS_SBO(Float)),
      val(v)
{
    Float::updateCnt();
}
...
Ob*
checkFlResult (Prim* prim, Ctxt* ctxt, Rfloat answer)
{
    if (arithmeticException) {
	arithmeticException = 0;
	return prim->runtimeError(ctxt, "arithmetic exception");
...
DEF("fl-",flMinus, 1, 2)
{
    ...
    switch (NARGS) {
      ...
      case 2: {
	  CHECK(0, Float, m); CHECK(1, Float, n);
