...
FixupVec::FixupVec (int n)
    : BinaryOb(sizeof(FixupVec) + n*sizeof(FixupEntry), CLASS_META(FixupVec), CLASS_SBO(FixupVec)),
      count(0)
{
    FixupVec::updateCnt();
}
...
FixupVec::FixupVec (FixupVec* oldvec, int newsize)
    : BinaryOb(sizeof(FixupVec) + newsize*sizeof(FixupEntry), oldvec->meta(), oldvec->parent()),
      count(oldvec->count)
{
    memcpy(&entry(0), &oldvec->entry(0), min(oldvec->count, newsize)*sizeof(FixupEntry));
}


FixupVec*
FixupVec::create (int n)
{
...
FixupVec*
FixupVec::create (FixupVec* oldvec, int newsize)
{
    void* loc = PALLOC1(sizeof(FixupVec) + newsize*sizeof(FixupEntry), oldvec);
    return NEW(loc) FixupVec (oldvec, newsize);
}


void
FixupVec::addEntry (int loc, Label label)
{
    int n = count++;
    entry(n).loc = loc;
    entry(n).label = label;
...
LabelTable*
LabelTable::create (int size)
{
    Word16Vec* lv = Word16Vec::create (size);	PROTECT(lv);
    FixupVec* fv = FixupVec::create (size);	PROTECT(fv);
...
Label
LabelTable::newExternalLabel (LabelNode* label_node)
{
    PROTECT_THIS(LabelTable); PROTECT(label_node);
...
void
LabelTable::addFixup (int loc, Label label)
{
...
void
LabelTable::bindLabel (Label label, int loc)
{
...
Label
LabelTable::getLabel (Ob* label_name)
{
...
LabelNode*
LabelTable::getLabelNode (Ob* label_name)
{
    Tuple* pair = (Tuple*) externalLabels->getKey(label_name);
...
int
LabelTable::resolveLabel (Label label)
{
...
void
LabelTable::resolveLabels (CodeBuf* cb)
{
    int n = fixupValues->numberOfEntries();
    for (int i = 0; i < n; i++) {
