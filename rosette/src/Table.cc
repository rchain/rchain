...
BUILTIN_CLASS(RblTable)
{
    OB_FIELD("tbl", RblTable, tbl);
};
...
RblTable*
RblTable::create (RblTableHitFn rtabhfn, int max)
{
    Tuple* tmp = Tuple::create (max*(sizeof(Entry)/sizeof(pOb)), ABSENT);
    void* loc = PALLOC1(sizeof(RblTable), tmp);
...
void
RblTable::rehashCompletelyFrom (int n)
{
    for (; n < maxEntries; n++) {
	/*
...
void
RblTable::linearAdd (pOb key, pOb val)
{
...
void
RblTable::hashAdd (pOb key, pOb val)
{
    int numberOfProbes = maxEntries;
    int probe = hash(key) % maxEntries;
...
void
RblTable::addEntry (pOb key, pOb val)
{
    (this->*addFn)(key, val);
}
...
int
RblTable::traversePtrs (PSOb__PSOb f)
{
...
void
RblTable::traversePtrs (V__PSOb f)
{
...
pOb
RblTable::cloneTo (pOb new_meta, pOb new_parent)
{
    PROTECT_THIS(RblTable);
...
RblTable::dumpPairs ()
{
    ...
    if (SELF->addFn == &RblTable::linearAdd)
	memcpy(&result->elem(0), &SELF->entry(0), SELF->numberOfEntries*sizeof(Entry));
    else {
	int i = 0;
	for (int j = maxEntries; j--;) {
	    pOb key = SELF->entry(j).key;
...
pOb
RblTable::addKey (pOb key, pOb val)
{
  PROTECT_THIS(RblTable); PROTECT(key); PROTECT(val);
