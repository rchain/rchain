...
inline
Expr::Expr (int sz, Ob* meta, Ob* parent)
    : Ob(sz, meta, parent)
{ }
...
BlockExpr*
BlockExpr::create (Tuple* init, Ob* implicit)
{
...
Ob* BlockExpr::setNth (int n, Ob* x)	{ ASSIGN(subExprs, elem(n), x); return this; }
Ob* BlockExpr::subObject (int i, int n)	{ return BlockExpr::create (subExprs->makeSlice(i, n)); }
...
FreeExpr*
FreeExpr::create (TupleExpr* freeIds, Ob* body)
{
...
BUILTIN_CLASS(GotoExpr)
{
    OB_FIELD("label", GotoExpr, label);
}
...
LabelExpr*
LabelExpr::create (Ob* label, Ob* body)
{
...
LetExpr*
LetExpr::create (TupleExpr* te, Ob* b)
{
...
Ob* LetExpr::boundExpr (int n)	{ return ((TupleExpr*)bindings->elem(n))->elem(1); }
...
LetrecExpr::LetrecExpr (TupleExpr* te, Ob* b)
    : LetExpr(sizeof(LetrecExpr), CLASS_META(LetrecExpr), CLASS_SBO(LetrecExpr), te, b)
{
...
LetrecExpr*
LetrecExpr::create (TupleExpr* te, Ob* b)
{
...
MethodExpr*
MethodExpr::create (Ob* i, Ob* f, Ob* b)
{
...
ProcExpr::ProcExpr (Ob* i, Ob* f, Ob* b)
    : MethodExpr(sizeof(ProcExpr), CLASS_META(ProcExpr), CLASS_SBO(ProcExpr), i, f, b)
{
    ProcExpr::updateCnt();
}
...
ProcExpr*
ProcExpr::create (Ob* i, Ob* f, Ob* b)
{
...
BUILTIN_CLASS(QuoteExpr)
{
    OB_FIELD("expr",	QuoteExpr, expr);
}


QuoteExpr::QuoteExpr (Ob* e)
    : Expr(sizeof(QuoteExpr), CLASS_META(QuoteExpr), CLASS_SBO(QuoteExpr)), expr(e)
{
  QuoteExpr::updateCnt();
}
...
ReflectiveMethodExpr::ReflectiveMethodExpr (Ob* i, Ob* f, Ob* b)
    : MethodExpr(sizeof(ReflectiveMethodExpr),
		 CLASS_META(ReflectiveMethodExpr),
		 CLASS_SBO(ReflectiveMethodExpr),
		 i, f, b)
{
...
ReflectiveMethodExpr*
ReflectiveMethodExpr::create (Ob* i, Ob* f, Ob* b)
{
...
RequestExpr::RequestExpr (Ob* t, TupleExpr* te)
    : Expr(sizeof(RequestExpr), CLASS_META(RequestExpr), CLASS_SBO(RequestExpr)),
      target(t), msg(te)
{
...
RequestExpr*
RequestExpr::create (Ob* t, TupleExpr* te)
{
...
SendExpr::SendExpr (Ob* t, TupleExpr* m)
    : RequestExpr(sizeof(SendExpr), CLASS_META(SendExpr), CLASS_SBO(SendExpr), t, m)
{
    SendExpr::updateCnt();
}
...
SendExpr*
SendExpr::create (Ob* t, TupleExpr* m)
{
...
BUILTIN_CLASS(SeqExpr)
{
    OB_FIELD("sub-exprs", SeqExpr, subExprs);
}
...
Ob* SeqExpr::setNth (int n, Ob* x)	{ ASSIGN(subExprs, elem(n), x); return this; }
Ob* SeqExpr::subObject (int i, int n)	{ return SeqExpr::create (subExprs->makeSlice(i, n)); }
...
BUILTIN_CLASS(TupleExpr)
{
    OB_FIELD("rest", TupleExpr, rest);
}
...
TupleExpr::TupleExpr (int n, Ob* r)
    : Expr(sizeof(TupleExpr)+n*sizeof(Ob*), CLASS_META(TupleExpr), CLASS_SBO(TupleExpr)),
      rest(r)
{
    /*
...
TupleExpr*
TupleExpr::create (Ob** p, int n, Ob* r)
{
    if (n == 0 && r == NILexpr)
	return NILexpr;
...
TupleExpr::create (int n, Ob* r)
{
	...
	void* loc = PALLOC1(sizeof(TupleExpr) + n*sizeof(Ob*), r);
	TupleExpr* result = NEW(loc) TupleExpr (n, r);
	for (int i = 0; i < n; i++) result->elem(i) = INVALID;
	return result;
    }
    else {
	TupleExpr* rp = (TupleExpr*) r;
...
TupleExpr::allPairs ()
{
	...
	if (!IS_A(elem(i), TupleExpr) || ((TupleExpr*)elem(i))->numberOfElements() != 2)
	    return FALSE;
    return TRUE;
}
...
TupleExpr*
TupleExpr::cons (Ob* val)
{
    PROTECT_THIS(TupleExpr); PROTECT(val);
...
TupleExpr*
TupleExpr::makeSlice (int start, int span)
{
...
Ob*
TupleExpr::cloneTo (Ob* new_meta, Ob* new_parent)
{
...
Ob*
TupleExpr::indexedSize ()
{
    return FIXNUM(numberOfElements());
}
...
Ob*
TupleExpr::subObject (int i, int n)
{
    return makeSlice(i, n);
}



Ob*
blockify (int start, int n, Tuple* exprs)
{
    if (n == 1)
...
DEF("tupleexpr-new-n",tplexprNewN, 4, 4)
{
    ...
    for (int i = 0; i < n; i++ ) result->elem(i) = ARG(3);
    return result;
}
...
DEF("tupleexpr-new",tplexprNew, 3, 3)
{
    ...
    PROTECT(__CTXT__); PROTECT(subExprs);
...
DEF("tupleexpr-split",tplexprSplit, 2, 2)
{
  ...
  TupleExpr* r; PROTECT(expr); PROTECT(r);
  if (s == n) {
