...
bool
Ob::isCoveredByp(pOb x)
{ return BASE(x)->compositeCoversp(this->self()); }
...
bool
Ob::typeMatchesp(pOb x)
{ return this->coversp(x); }

 ...
 */
bool
Tuple::typeMatcher (Tuple* type_template, pOb rest_type)
{
...
bool
Tuple::elemsCoveredByp (pOb typ, int i)
{
  int N = numberOfElements();
...
BUILTIN_CLASS(MIActor)
{
    OB_FIELD_INDIRECT("cpl", MI_CPL_SLOT);
}
...
MIActor*
MIActor::create (Tuple* class_precedence_list)
{
...
Ob*
MIActor::lookup (Ob* key, Ctxt* ctxt)
{
    if (interruptPending)
	return ABSENT;
	...
	Ob* result = BASE(effective_base->meta())->get(effective_base, key, ctxt);
	if (result != ABSENT)
	    return result;
    }
...
MIActor::typeLub (pOb x)
{
  if (this->coversp(x))
	...
	PROTECT(cpl); PROTECT(x);
	pOb u = BASE(x)->typeLub(cpl->elem(0));
	PROTECT(u);
	for (int i = 1; i < N; i++) {
	  pOb v = BASE(x)->typeLub(cpl->elem(i));
	  if (BASE(u)->coversp(v)) /* does coversp ever do a typeLub ?? */
	    u = v;
...
: Actor(sizeof(ProductType),
	CLASS_META(ProductType),
	CLASS_SBO(ProductType),
	lockedMbox, ext)
{
...
ProductType*
ProductType::create (Tuple* type_template, pOb rest_type)
{
  PROTECT(type_template); PROTECT(rest_type);
...
bool
ProductType::typeMatchesp (pOb actuals)
{
...
bool
ProductType::isCoveredByp (pOb x)
{
...
BUILTIN_CLASS(SumType)
{
  OB_FIELD_INDIRECT("types", SUM_TYPE_TYPES_SLOT);
}


SumType::SumType (pExt ext)
: Actor(sizeof(SumType),
	CLASS_META(SumType),
	CLASS_SBO(SumType),
	lockedMbox, ext)
{
...
SumType*
SumType::create (Tuple* sum_types)
{
  PROTECT(sum_types);
...
bool
SumType::isCoveredByp (pOb X)
{
  /* a sum is covered by X iff EVERY elem of types is covered by X */
  for (int i = 0; i < numberOfElements(); i++)
      if (!BASE(X)->coversp(elem(i))) return FALSE;

  return TRUE;
}

bool
SumType::compositeCoversp (pOb X)
{
  /* a SumType covers X iff SOME elem of types covers X */
  for (int i = 0; i < numberOfElements(); i++)
    if (BASE(elem(i))->coversp(X)) return TRUE;
...
SumType::dominator ()
{
  return tupleLub(types());
}

pOb
SumType::typeLub (pOb x)
{
  if (this->coversp(x))
    return this;
  else if (BASE(x)->coversp(this))
...
BUILTIN_CLASS(MultiMethod)
{
  OB_FIELD_INDIRECT("procList", MM_PROC_LIST_SLOT);
}


MultiMethod::MultiMethod (pExt ext)
: Actor(sizeof(MultiMethod),
	CLASS_META(MultiMethod),
	CLASS_SBO(MultiMethod),
	lockedMbox, ext)
{
...
pOb
MultiMethod::matchAndDispatch (pCtxt ctxt)
{
  Proc* p;
  Tuple* pid;
  pOb typ;
...
DEF("type<",obTypeLessp, 2, 2)
{
  return RBLBOOL(typeLess(ARG(0), ARG(1)));
}

DEF("type<>",obTypesIncomparable, 2, 2)
{
  return RBLBOOL(!(typeLessEq(ARG(0), ARG(1)) ||
		   typeLessEq(ARG(1), ARG(0))));
}
...
DEF("typeLub",obsTypeLub, 1, MaxArgs)
{
  return tupleLub(ARGS);
}
...
DEF("typeDominator",obTypeDominator, 1, 1)
{
  return typeDominator(ARG(0));
}
