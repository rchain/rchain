...
{
  CHECK_FIXNUM(1, base);      
  CHECK(2, Tuple, accessors); PROTECT(accessors);
  return( ((GenericDescriptor*)(BASE(ARG(0))))->select( __CTXT__, base, accessors ) );
}
...
DEF("S-get",obSGet, 3, 3)
{
  CHECK_FIXNUM(1, base);      
  CHECK(2, Tuple, accessors); PROTECT(accessors);
  return( ((GenericDescriptor*)(BASE(ARG(0))))->sGet( __CTXT__, base, accessors ) );
}
...
DEF("S-desc",obSDesc, 3, 3)
{
  CHECK_FIXNUM(1, base);      
  CHECK(2, Tuple, accessors); PROTECT(accessors);
  return( ((GenericDescriptor*)(BASE(ARG(0))))->sDesc( __CTXT__, base, accessors ) );
}
...
DEF("S-deref",obSDeref, 3, 3)
{
  CHECK_FIXNUM(1, base);      
  CHECK(2, Tuple, accessors); PROTECT(accessors);
  return( ((GenericDescriptor*)(BASE(ARG(0))))->sDeref( __CTXT__, base, accessors ) );
}
...
DEF("S-set",obSSet, 3, 3)
{
  CHECK_FIXNUM(1, base);      
  CHECK(2, Tuple, accessors); PROTECT(accessors);
  Ob* val = TUPLE_HEAD(accessors,0); PROTECT(val);
  return( ((GenericDescriptor*)
	   (BASE(ARG(0))))->sSet( __CTXT__, base, val, accessors, 1 ) );
}

DEF("S-tupleSet",obSTupleSet, 4, 4)
{
  CHECK_FIXNUM(1, base);      
  CHECK(2, Tuple, val); PROTECT(val);
  CHECK(3, Tuple, accessors); PROTECT(accessors);
  return( ((GenericDescriptor*)(BASE(ARG(0))))->sTupleSet( __CTXT__, base, val, accessors ) );
}
...
{
  return( FIXNUM((((GenericDescriptor*)(BASE(ARG(0))))->convertActualArg(__CTXT__, ARG(1))).val) );
}

DEF(">-A-",obConvertActualRslt, 2, 2)
{
  CHECK_FIXNUM(1, rslt);
  return( ((GenericDescriptor*)(BASE(ARG(0))))->convertActualRslt(__CTXT__, rslt) );
}

DEF("csNth",obCSNth, 3, 3)
{
  CHECK_FIXNUM(1,base);
  CHECK(2, Tuple, ipath); PROTECT(ipath);
  Ob* val = TUPLE_HEAD(ipath,0); PROTECT(val);
  return( ((GenericDescriptor*)
	   (BASE(ARG(0))))->nthBase(__CTXT__, base, FIXVAL(val), ipath, 0) );
}
...
: Actor(sizeof(GenericDescriptor), CLASS_META(GenericDescriptor),
	...
	_offset(0), _align_to(0), _size(0),
	mnemonic(NIV), imported(RBLFALSE), freeStructOnGC(RBLFALSE)
{
...
int
GenericDescriptor::traversePtrs( PSOb__PSOb f )
{
  int sum = 0;
...
int
GenericDescriptor::traversePtrs( SI__PSOb f )
{
  int sum = 0;
...
void
GenericDescriptor::traversePtrs( V__PSOb f )
{
  useIfPtr(meta(), f);
  useIfPtr(parent(), f);
  useIfPtr(mbox, f);
...
Ob* GenericDescriptor::oprnSwitch( Ctxt* ctxt, Word32 base, Tuple* path, int pindex )
{
  PROTECT_THIS(GenericDescriptor);
  PROTECT(ctxt); PROTECT(path);
  ...
  if (head == oprnSDeref)
    ...
    if (head == oprnCSNth)
      return SELF->nthBase( ctxt, base,
			   FIXVAL(TUPLE_HEAD(path,pindex)),
			   path, pindex );
    else {
      TUPLE_TAIL(path,pindex);
      if (head == oprnSSet)
	return SELF->sSet( ctxt, base,
			  TUPLE_HEAD(path,(pindex - 1)),
			  path, pindex );
...
Ob*
GenericDescriptor::nullDescriptor( Ctxt* ctxt )
{
  ...
  if ((rslt = mta->get( prnt, oprnNull, ctxt )) == ABSENT) {
    ...
    if (knd == ABSENT)
      ...
      Ob* nullactid = SYMBOL(nullactstr); PROTECT(nullactid);

      Ob* ns = newSBO(obSBO, nullactid, CLASS_SBO(NullDescriptor), ctxt);
      PROTECT(ns); 
      Ob* nd = genActor(obNullDescriptor, ns); PROTECT(nd);
...
GenericDescriptor::convertActualArg( Ctxt* ctxt, Ob* obj )
{
...
Ob* GenericDescriptor::sGet( Ctxt* ctxt, Word32 base, Tuple* path, int pindex )
{
  ...
  } else return oprnSwitch( ctxt, base, path, pindex );
}
...
Ob* GenericDescriptor::sSet( Ctxt* ctxt, Word32 base, Ob* val, Tuple* path, int pindex )
{
  if (TYPEP(this, val)) {
    memcpy( (void*)(base + _offset), (void*)(((GenericDescriptor*)val)->_offset), (int)_size );
    if (NULLP(path,pindex)) {
      return NIV;
    } else
...
Ob*
GenericDescriptor::sTupleSet( Ctxt* ctxt, Word32 base, Tuple* val, Tuple* path, int pindex )
{
  // Needs protection code.
  PROTECT_THIS(GenericDescriptor);
  PROTECT(ctxt); PROTECT(path); PROTECT(val);
  int vindex = 0;
  Ob* tupHead = INVALID; PROTECT(tupHead);
  
  for (Word32 addr = base;
       !(NULLP(val,vindex));
...
Ob*
GenericDescriptor::nthBase( Ctxt* ctxt, Word32 base,
			   int i, Tuple* path, int pindex )
{
  PROTECT_THIS(GenericDescriptor);
  PROTECT(ctxt); PROTECT(path);
...
Word32
GenericDescriptor::absoluteAddress( Word32 base )
{
  Word32 newbase = (base + _offset);
  Word32 newoff = newbase % 4;
  return (Word32)(mem_get_field(
				(Word32*)(newbase - newoff),
				(int)(newoff * 8),
				(int)(_size * 8),
				BOOLVAL(RBLFALSE)
				));
...
void
GenericDescriptor::setAddrContents( Word32 base, Word32 val )
{
  Word32 newbase, offset, *addr;
  newbase = base + _offset;
  offset = newbase % 4;
    
  mem_set_field((Word32*)(newbase - offset),
		(int)(offset * 8),
		(int)(_size * 8),
		val);
}
...
Ob* NullDescriptor::flatten( Ctxt*, Word32, RblTable* )
{
  return this;
}

Word32
NullDescriptor::absoluteAddress( Word32 base )
{
  return (Word32)0;
}
...
AtomicDescriptor*
AtomicDescriptor::create(RblBool* b)
{
  PROTECT(b);
  StdExtension* ext = StdExtension::create(0);
...
convertArgReturnPair
AtomicDescriptor::convertActualArg( Ctxt* ctxt, Ob* obj )
{
...
Ob* AtomicDescriptor::sGet( Ctxt* ctxt, Word32 base, Tuple* path, int pindex )
{
  ...
  if (NULLP(path,pindex)) {    
    return( FIXNUM(SELF->absoluteAddress(base)) );
  } else return( SELF->oprnSwitch( ctxt, base, path, pindex ) );
}

Ob* AtomicDescriptor::sSet( Ctxt* ctxt, Word32 base, Ob* val, Tuple* path, int pindex )
{
  // Needs protection code.
  PROTECT_THIS(AtomicDescriptor);
  PROTECT(ctxt); PROTECT(val);
  PROTECT(path);
  ...
  } else return runtimeError( ctxt, "S-set type-mismatch ", val );
}
...
Word32
AtomicDescriptor::absoluteAddress( Word32 base )
{
  Word32 newbase = (base + _offset);
  Word32 newoff = newbase % 4;
  return (Word32)(mem_get_field(
...
GenericDescriptor(sizeof(CStructure), CLASS_META(CStructure),
		  ...
		  _descs(desks), _fieldNames(fnames)
{
...
CStructure*
CStructure::create(RblTable* tbl, Tuple* tup)
{
  PROTECT(tbl); PROTECT(tup);
  StdExtension* ext = StdExtension::create(0);
  ...
  if (NULLP(path,pindex))
    {
      ...
      Ob* symb = TUPLE_HEAD(path,pindex); PROTECT(symb);
      TUPLE_TAIL(path,pindex);
	...
	return (SELF->sBox( base + SELF->_offset + d->_offset ));
      } else return d->oprnSwitch( ctxt, (base + SELF->_offset), path, pindex );
    }
}

Ob*   CStructure::sTupleSet( Ctxt* ctxt, Word32 base, Tuple* val, Tuple* path, int pindex )
{
  // Needs protection code.
  
  PROTECT_THIS(CStructure);
  PROTECT(ctxt); PROTECT(val); PROTECT(path);
  ...
  if (val->numberOfElements() <= SELF->_fieldNames->numberOfElements()) {
    ...
    int vindex = 0;
    Ob* x = TUPLE_HEAD(val,vindex); PROTECT(x);
    for( int i = 0;
	!(NULLP(val,vindex));
	i++) {
      TUPLE_TAIL(val,vindex);
      entry =
	(GenericDescriptor*)(SELF->_descs->getKey( SELF->_fieldNames->elem( i ) ));
      entry->sSet( ctxt, (base + SELF->_offset), x, NIL );
      x = TUPLE_HEAD(val,vindex);
    }
    return NIV;
  } else return runtimeError( ctxt, "S-tupleSet too many elements ", val );
}

Ob* CStructure::flatten( Ctxt* ctxt, Word32 base, RblTable* occtxt )
{
  PROTECT_THIS(CStructure); PROTECT(ctxt); PROTECT(occtxt);
  Ob* chck = INVALID; PROTECT(chck);
  ...
  if ((chck = occtxt->getKey(selfTag)) == ABSENT)
    {
      ...
      PROTECT(symb); PROTECT(tmp); PROTECT(crc);
...
GenericDescriptor(sizeof(CArray), CLASS_META(CArray),
		  CLASS_SBO(CArray), emptyMbox, ext),
		  _numElems(cnt), _elemDesc(elemd)
{
 CArray::updateCnt();
}

CArray::CArray(int s, pOb m, pOb p, pOb mbx, pExt ext,
	       Word16 cnt, GenericDescriptor* elemd) :
...
CArray*
CArray::create(Word16 cnt, GenericDescriptor* elmd)
{
  PROTECT(elmd);
  StdExtension* ext = StdExtension::create(0);
...
Ob*   CArray::sTupleSet( Ctxt* ctxt, Word32 base, Tuple* val, Tuple* path, int pindex )
{
...
Ob*   CArray::nthBase( Ctxt* ctxt, Word32 base, int i, Tuple* path, int pindex )
{
  ...
  } else return runtimeError( ctxt, "Index out of bounds ", path );
}

Ob* CArray::flatten( Ctxt* ctxt, Word32 base, RblTable* occtxt )
{
  PROTECT_THIS(CArray); PROTECT(ctxt); PROTECT(occtxt);
  Ob* chck = INVALID; PROTECT(chck);
  ...
  if ((chck = occtxt->getKey(selfTag)) == ABSENT)
  {
    ...
    for (int i = 0; i < SELF->_numElems; i++) {
      Word32 addrKey = base + SELF->_offset + (i * sizeof(SELF->_elemDesc));
...
CArray(sizeof(CharArray), CLASS_META(CharArray),
       CLASS_SBO(CharArray), emptyMbox, ext, cnt, elemd)
{
 CharArray::updateCnt();
}

CharArray::CharArray(int s, pOb m, pOb p, pOb mbx, pExt ext,
		     Word16 cnt,GenericDescriptor* elemd) :
...
CharArray*
CharArray::create(Word16 cnt, GenericDescriptor* elmd)
{
  PROTECT(elmd);
  StdExtension* ext = StdExtension::create(0);
...
Ob* CharArray::sSet( Ctxt* ctxt, Word32 base, Ob* val, Tuple* path, int pindex )
{
  if (STRINGP(val)) {
    ...
    } else return runtimeError( ctxt, "invalid address ", FIXNUM(addr) );
    return NIV;
  } else return CArray::sSet( ctxt, base, val, path, pindex );
}

Ob* CharArray::flatten( Ctxt* ctxt, Word32 base, RblTable* occtxt )
{
  PROTECT_THIS(CharArray); PROTECT(ctxt); PROTECT(occtxt);
  Ob* chck = INVALID; PROTECT(chck);
  Ob* selfTag = makeOCTag( SELF, base + SELF->_offset );
  PROTECT(selfTag);
  ...
  if ((chck = occtxt->getKey( selfTag )) == ABSENT)
  {
    ...
    else return runtimeError( ctxt, "invalid address");
  } else return chck;
}
...
CharArray0*
CharArray0::create(Word16 cnt, GenericDescriptor* elmd)
{
  PROTECT(elmd);
  StdExtension* ext = StdExtension::create(0);
...
GenericDescriptor(sizeof(CRef), CLASS_META(CRef),
		  CLASS_SBO(CRef), emptyMbox, ext),
		  _desc(elemd)
{
 CRef::updateCnt();
}
...
CRef*
CRef::create(GenericDescriptor* elemd)
{
  PROTECT(elemd);
  StdExtension* ext = StdExtension::create(0);
...
convertArgReturnPair
CRef::convertActualArg( Ctxt* ctxt, Ob* obj )
{
  PROTECT_THIS(CRef); PROTECT(ctxt); PROTECT(obj);
  cnvArgRetPair.failp =0;
...
Ob*   CRef::sDeref( Ctxt* ctxt, Word32 base, Tuple* path, int pindex )
{
  ...
  GenericDescriptor* d =
    ((bddr == 0) ? (GenericDescriptor*)(_desc->nullDescriptor(ctxt)) : _desc);
  
  if (NULLP(path,pindex))
    return( d->sBox(bddr) );
  else return( d->oprnSwitch( ctxt, bddr, path, pindex ) );
}
...
Ob*   CRef::sSet( Ctxt* ctxt, Word32 base, Ob* val, Tuple* path, int pindex )
{
  if (TUPLEP(val)) {
    ...
    if (TYPEP(this,val)) {
      ...
      msetval = 0;
    } else return runtimeError( ctxt, "wrong type for assignment ", val );
...
{
  PROTECT_THIS(CRef); PROTECT(ctxt); PROTECT(occtxt);
  Ob* chck = INVALID; PROTECT(chck);
  Ob* selfTag = makeOCTag( SELF, base + SELF->_offset );
  PROTECT(selfTag);
...
CRef(elemd, sizeof(CharRef), CLASS_META(CharRef),
		  CLASS_SBO(CharRef), emptyMbox, ext)
{
 CharRef::updateCnt();
}

CharRef::CharRef(GenericDescriptor* elemd, int s, pOb m, pOb p, pOb mbx, pExt ext) :
...
CharRef*
CharRef::create(GenericDescriptor* elemd)
{
  PROTECT(elemd);
  StdExtension* ext = StdExtension::create(0);
...
Ob*
CharRef::sSet( Ctxt* ctxt, Word32 base, Ob* val, Tuple* path, int pindex )
{
  PROTECT_THIS(CharRef); PROTECT(ctxt); PROTECT(val); PROTECT(path);
  if (STRINGP(val)) {
...
convertArgReturnPair
CharRef::convertActualArg(Ctxt* ctxt, Ob* arg)
{
...
CRef(elemd, sizeof(CRef0), CLASS_META(CRef0),
     CLASS_SBO(CRef0), emptyMbox, ext)
{
  CRef0::updateCnt();
}

CRef0::CRef0(GenericDescriptor* elemd,
	     int s, pOb mta, pOb prnt, pOb mbx, pExt ext) :
...
CRef0*
CRef0::create(GenericDescriptor* elemd)
{
  PROTECT(elemd);
  StdExtension* ext = StdExtension::create(0);
...
Ob* CRef0::sGet( Ctxt* ctxt, Word32 base, Tuple* path, int pindex )
{
  ...
  } else return runtimeError( ctxt, "cannot access ", path );
}

Ob* CRef0::flatten( Ctxt* ctxt, Word32 base, RblTable* occtxt )
{
  PROTECT_THIS(CRef0); PROTECT(ctxt); PROTECT(occtxt);
  Ob* chck = INVALID; PROTECT(chck);
  Ob* selfTag = makeOCTag( SELF, base + SELF->_offset );
...
CRef0((GenericDescriptor*)RBLFALSE, sizeof(CharRef0), CLASS_META(CharRef0),
      CLASS_SBO(CharRef0), emptyMbox, ext)
{
  CharRef0::updateCnt();
}
...
{
  PROTECT_THIS(CharRef0); PROTECT(ctxt); PROTECT(occtxt);
...
Ob* CharRef0::sSet( Ctxt* ctxt, Word32 base, Ob* val, Tuple* path, int pindex )
{
  if (STRINGP(val)) {
    ...
    return NIV;
  } else return CRef0::sSet( ctxt, base, val, path, pindex );
}

convertArgReturnPair
CharRef0::convertActualArg(Ctxt* ctxt, Ob* arg)
{
  GenericDescriptor* obj = (GenericDescriptor*)arg;
  cnvArgRetPair.failp = 0;
  ...
  if  (STRINGP(obj)) {
    cnvArgRetPair.val = (Word32)(GET_STRING(obj));
  } else return CRef::convertActualArg( ctxt, arg );
...
GenericDescriptor(sizeof(CUnion), CLASS_META(CUnion),
		  CLASS_SBO(CUnion), emptyMbox, ext),
		  _descs(desks), _fieldNames(fnames)
{
  CUnion::updateCnt();
}
...
CUnion*
CUnion::create(RblTable* tbl, Tuple* tup)
{
  PROTECT(tbl); PROTECT(tup);
  StdExtension* ext = StdExtension::create(0);
...
{
  PROTECT_THIS(CUnion); PROTECT(ctxt); PROTECT(occtxt);
  Ob* chck = INVALID; PROTECT(chck);
  Ob* selfTag = makeOCTag( SELF, base + SELF->_offset );
...
MODULE_INIT(Cstruct)
{
  obGenericDescriptor = (GenericDescriptor*)(heap->tenure( GenericDescriptor::create() ));
  Define( "GenericDescriptor", obGenericDescriptor );
  
  obNullDescriptor = (NullDescriptor*)(heap->tenure( NullDescriptor::create() ));
  Define( "NullDescriptor", obNullDescriptor );
  
  obAtomicDescriptor = (AtomicDescriptor*)(heap->tenure( AtomicDescriptor::create() ));
