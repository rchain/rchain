...
StdExtension::StdExtension (int	nslots)
    : Ob(sizeof(StdExtension)+nslots*sizeof(pOb),
	 CLASS_META(StdExtension),
...
StdExtension*
StdExtension::create (pOb meta, pOb parent, int nslots)
{
    void* loc = PALLOC2(sizeof(StdExtension)+nslots*sizeof(pOb), meta, parent);
    return NEW(loc) StdExtension (meta, parent, nslots);
}


StdExtension*
StdExtension::create (int nslots)
{
...
StdExtension*
StdExtension::create (pTuple argvec)
{
...
Actor::Actor (pOb meta, pOb parent, pExt ext)
    : MboxOb(sizeof(Actor), meta, parent, lockedMbox),
      extension(ext)
{
    if (meta != INVALID)
	BASE(meta)->addRef();
...
Actor*
Actor::create (pOb meta, pOb  parent, pExt  ext)
{
...
pOb
Actor::cloneTo (pOb new_meta, pOb new_parent)
{
    PROTECT_THIS(Actor); PROTECT(new_parent); PROTECT(new_meta);
...
pOb
Actor::update (bool  enabled_set_provided, pCtxt ctxt)
{
    PROTECT_THIS(Actor); PROTECT(ctxt);
    pOb result = SELF;
    PROTECT(result);
...
pOb
Actor::updateByLoc (bool  enabled_set_provided, pCtxt ctxt)
{
    PROTECT_THIS(Actor); PROTECT(ctxt);
    pOb result = SELF;
    PROTECT(result);
...
pOb
Actor::becomeNew (pOb proto, pCtxt ctxt)
{
    PROTECT_THIS(Actor); PROTECT(proto); PROTECT(ctxt);
    proto = BASE(proto);
...
pOb
Actor::lookupAndInvoke (pCtxt ctxt)
{
...
TblObject::TblObject (pExt ext, pOb validExtent, pTuple keyVec)
    : Actor(sizeof(TblObject), CLASS_META(TblObject), CLASS_SBO(TblObject), emptyMbox, ext),
...
int
TblObject::addSlot (pOb key, pOb val)
{
...
pOb
TblObject::entryKey (int n)
{
    return keyVec->elem(n);
}
...
DEF("actor-new",actorNew, 3, 3)
{
    ...
    CHECK(1, Tuple, keyVec);	PROTECT(keyVec);
    CHECK(2, Tuple, vals);	PROTECT(vals);
    pOb sbo = ARG(0);		PROTECT(sbo);
