...
Ob*
RblStack::cloneTo (Ob* new_meta, Ob* new_parent)
{
...
void
RblStack::push (Ob* val)
{
    int N = FIXVAL(nElems);

    if (N == elems->numberOfElements()) {
	PROTECT_THIS(RblStack); PROTECT(val);
...
Ob* RblStack::pop ()
{
	...
	PROTECT_THIS(RblStack); PROTECT(result);
...
Ob*	RblStack::setNth (int n, Ob* val)	{ ASSIGN(elems, elem(n), val); return this; }


Ob*
RblStack::subObject (int start, int size)
{
    PROTECT_THIS(RblStack);
...
DEF("stack-new",stackNew, 0, 0)
{
    return RblStack::create ();
}
