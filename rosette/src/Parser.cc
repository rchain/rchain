...
int 
debug_builtinprim(char *s)
{  /* printf("listing %s\n", s); */
}


inline void*
ParserFrame::operator new (size_t s)
{
...
inline void*
ParserFrame::operator new (size_t, void* p)
{
    return p;
}
...
inline
int
ParseTable::isDelimiter (int c)
{
    return attributes[c] & _DELIMITER;
}


inline
void*
Parser::falloc (int sz)
{
    return fstk.alloc(sz);
}


inline
int
PFrameStk::link (int sz)
{
    /*
...
PFrameStk::top ()
{
    return (ParserFrame*) &stk[topframe];
}
...
inline
ParserMode
Parser::accept (int c, int gapsPermitted)
{
  if (c == EOF)
    return STOP;
  else if (c == '\\')
...
inline
ParserFrame::ParserFrame (Parser* r, int sz)
    : link(r->flink(sz))
{ }

ParserMode
ParserFrame::process (int, Parser*)
{
...
ParserMode
ParserFrame::receiveOb (Ob*, Parser*)
{
...
ParserMode
ParserFrame::receiveChar (int, Parser*)
{
...
ParserMode
ParserFrame::receiveTerminator (int c, Parser* r)
{
...
ParserMode
ParserFrame::receiveDot (int, Parser*)
{
...
ParserMode
ParserFrame::receiveEof (Parser* r)
{
...
CommentFrame::CommentFrame (Parser* r)
    : ParserFrame (r, sizeof(CommentFrame))
{ }


ParserMode
CommentFrame::process (int c, Parser* r)
{
    if (c == EOF || c == '\n') {
	r->fpop();
	return START;
...
EscCharFrame::EscCharFrame (Parser* r, int gp)
    : ParserFrame(r, sizeof(EscCharFrame)),
      gapPermitted(gp), nchars(0), base(0), val(0)
{ }


ParserMode
EscCharFrame::process (int c, Parser* r)
{
    if (base == 0) {
	switch (c) {
	  case 'n':
...
inline
AtomFrame::AtomFrame (Parser* r, int sz)
    : ParserFrame(r, sz)
{ }


AtomFrame::AtomFrame (Parser* r)
    : ParserFrame(r, sizeof(AtomFrame))
{ }


ParserMode
AtomFrame::process (int c, Parser* r)
{
...
ParserMode
AtomFrame::receiveChar (int c, Parser* r)
{
    r->buffer(c);
    return CONTINUE;
}
...
ParserMode
StringFrame::process (int c, Parser* r)
{
    if (c == delimiter) {
	PROTECT(r);
...
ParserMode
SpecialPFrame::process (int c, Parser* r)
...
Ob*
SpecialPFrame::checkSym (char* sym)
{
    /*
...
ParserMode
SpecialPFrame::receiveOb (Ob* v, Parser* r)
{
    assert(IS_SYM(v));
...
ParserMode
SpecialPFrame::receiveChar (int c, Parser* r)
{
    return r->receiveOb(RBLCHAR(c));
}
...
QuoteFrame::QuoteFrame (Parser* r)
    : ParserFrame(r, sizeof(QuoteFrame))
{ }


ParserMode
QuoteFrame::receiveOb (Ob* v, Parser* r)
{
    PROTECT(r);
...
TopFrame::TopFrame (Parser* r)
    : ParserFrame(r, sizeof(TopFrame))
{ }


ParserMode
TopFrame::receiveOb (Ob* result, Parser* r)
{
    r->fpop();
    r->opush(result);
    return STOP;
...
ParserMode
TopFrame::receiveEof (Parser* r)
{
    return receiveOb(RBLEOF, r);
}
...
ListFrame::ListFrame (Parser* r, int dc, int cc, FINALIZER f)
    : ParserFrame(r, sizeof(ListFrame)), nexprs(0), dotState(0),
      dotChar(dc), closingChar(cc), finalizer(f)
{ }


ParserMode
ListFrame::receiveOb (Ob* subexpr, Parser* r)
{
    if (dotState == 0) {
	++nexprs;
	r->opush(subexpr);
...
ParserMode
ListFrame::receiveTerminator (int c, Parser* r)
{
    if (c != closingChar)
	(void) r->error("unexpected closing '%c'", c);
    Ob* rest = NILexpr;
...
ParserMode
ListFrame::receiveDot (int c, Parser* r)
{
    if (c != dotChar)
...
ParserMode
WhitespaceParseMacro::start (int, Parser*)
{
    return START;
}
...
ParserMode
CommentParseMacro::start (int, Parser* r)
{
...
ParserMode
AtomParseMacro::start (int c, Parser* r)
{
#if defined(OPTIMIZE_ATOMS)
    /*
    ...
    r->mode = GROK_ATOM;
#else
    (void) NEW(r->falloc(sizeof(AtomParseMacro))) AtomFrame (r);
#endif
...
ParserMode
StringParseMacro::start (int c, Parser* r)
{
...
ParserMode
_SpecialParseMacro::start (int, Parser* r)
{
    (void) NEW(r->falloc(sizeof(SpecialPFrame))) SpecialPFrame (r);
    return CONTINUE;
}
...
ParserMode
QuoteParseMacro::start (int, Parser* r)
{
...
ParserMode
DotParseMacro::start (int c, Parser* r)
{
    return r->receiveDot(c);
}
...
ParserMode
ListTerminatorParseMacro::start (int c, Parser* r)
{
...
ParserMode
ListParseMacro::start (int, Parser* r)
{
    (void) NEW(r->falloc(sizeof(ListFrame))) ListFrame(r, dotChar, closingChar, finalizer);
    return START;
}
...
static
Ob*
msgFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n == 0 && rest == NILexpr)
	return NILexpr;
    else
...
static
Ob*
rqstFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n == 0)
...
static
Ob*
sendFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 2)
	return r->error("no target in send expr");

    return( SendExpr::create (stk[1],
			      (TupleExpr*) msgFinalizer(r, stk+2, n-2, rest)) );
}


static
Ob*
ifFinalizer (Parser* r, Ob** stk, int n, Ob*)
{
    switch (n) {
      case 4:
...
static
Ob*
blockHelper (Ob** stk, int n, bool implicit = TRUE)
{
    if (n == 1)
	return stk[0];
    else {
...
static
Ob*
blockFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 2 || rest != NILexpr)
...
static
Ob*
freeFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 3 || rest != NILexpr)
...
static
Ob*
methodFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 3 || rest != NILexpr)
...
static
Ob*
procFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 3 || rest != NILexpr)
...
static
Ob*
namedProcFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 4 || rest != NILexpr)
...
static
Ob*
letFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 3 || rest != NILexpr)
...
static
Ob*
letstarFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 3 || rest != NILexpr)
	return r->error("improper syntax for let* expression");
...
static
Ob*
letrecFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 3 || rest != NILexpr)
...
static
Ob*
seqFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    if (n < 2 || rest != NILexpr)
...
static
Ob*
setFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
...
static
Ob*
gotoFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
...
static
Ob*
labelFinalizer (Parser* r, Ob** stk, int n, Ob* rest)
{
    Ob* label;
...
void
ListParseMacro::install (ParseTable* rt)
{
...
ParseTable::ParseTable ()
{
    ...
    tbl[';']  = &_CRM;  attributes[';']  |= _DELIMITER;
    tbl['\"'] = &_SRM;  attributes['\"'] |= _DELIMITER;
    tbl['#']  = &_SpRM; attributes['#']  |= _DELIMITER;
    tbl['\''] = &_QRM;
...
Parser::Parser (ParseTable* rt)
    : BinaryOb(align(sizeof(Parser)), CLASS_META(Parser), CLASS_SBO(Parser)),
      rt(rt), fstk(), ostk(), mode(START), inbuf((RBLstring*) NIV), inp(0),
      buf(NULL), bufsize(0), bufp(0), waitingOnIO(NOT_WAITING),
...
Parser::resumeExpr ()
{
	...
	switch (nextMode) {
	  ...
	  case START:
	    if (c == EOF) /* shouldn't happen now - need to add an i/f */
...
ParserMode
Parser::receiveDot (int c)
{
    return ftop()->receiveDot(c, this);
}


ParserMode
Parser::receiveTerminator (int c)
{
...
ParserMode
Parser::acceptEscChar (int c, int gapsPermitted)
{
#if defined(OPTIMIZE_ATOMS)
    if (mode == GROK_ATOM) {
	/*
...
int
Parser::traversePtrs (PSOb__PSOb f)
{
  return (BinaryOb::traversePtrs(f)
	  + ostk.traversePtrs(f)
	  + useIfPtr(&inbuf, f));
}
...
int
Parser::traversePtrs (SI__PSOb f)
{
  return (BinaryOb::traversePtrs(f)
	  + ostk.traversePtrs(f)
	  + useIfPtr(inbuf, f));
