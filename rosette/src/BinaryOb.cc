...
int
BinaryOb::traversePtrs (PSOb__PSOb f)
{
...
int
BinaryOb::traversePtrs (SI__PSOb f)
{
    return useIfPtr(meta(), f) + useIfPtr(parent(), f);
}
...
void
BinaryOb::traversePtrs (V__PSOb f)
{
    useIfPtr(meta(), f);
    useIfPtr(parent(), f);
}
...
ByteVec::ByteVec (int sz)
    : BinaryOb(sizeof(ByteVec) + align(sz*sizeof(Byte)), CLASS_META(ByteVec), CLASS_SBO(ByteVec)),
      byteCount(sz)
{
...
ByteVec::ByteVec (ByteVec* old, int newsize)
    : BinaryOb(sizeof(ByteVec) + align(newsize*sizeof(Byte)), old->meta(), old->parent()),
      byteCount(newsize)
{
    int oldsize = old->numberOfBytes();
    memcpy((char*)&this->byte(0), (char*)&old->byte(0), min(oldsize, newsize)*sizeof(Byte));
    if (newsize > oldsize)
...
ByteVec*
ByteVec::create (ByteVec* old, int newsize)
{
...
ByteVec::reset (EMPTY)
{
    memset((char*)&byte(0), 0, numberOfBytes());
}
...
Ob*
ByteVec::indexedSize (EMPTY)
{
    return FIXNUM(numberOfBytes());
}
...
ByteVec::nth (int n)
{
    return FIXNUM(byte(n));
}


Ob*
ByteVec::setNth (int n, Ob* v)
{
    byte(n) = FIXVAL(v);
    return this;
}
...
Ob*
ByteVec::subObject (int start, int n)
{
    PROTECT_THIS(ByteVec);
    ByteVec* result = ByteVec::create (n);
    memcpy((char*)&result->byte(0), (char*)&SELF->byte(start), n*sizeof(Byte));
    return result;
}
...
Word16Vec::Word16Vec (int n)
    : BinaryOb(sizeof(Word16Vec) + align(n*sizeof(Word16)), CLASS_META(Word16Vec), CLASS_SBO(Word16Vec)),
      wordCount(n)
{
    while( n-- ) this->word(n) = 0;
    Word16Vec::updateCnt();
}
...
Word16Vec::Word16Vec (Word16Vec* old, int newsize)
    : BinaryOb(sizeof(Word16Vec) + align(newsize*sizeof(Word16)), old->meta(), old->parent()),
      wordCount(newsize)
{
    int oldsize = old->numberOfWords();
    memcpy(&this->word(0), &old->word(0), min(oldsize, newsize)*sizeof(Word16));
    if (newsize > oldsize)
...
Word16Vec*
Word16Vec::create (Ob* meta, Ob* parent, int n)
{
    void* loc = PALLOC2(sizeof(Word16Vec) + align(n*sizeof(Word16)), meta, parent);
    return NEW(loc) Word16Vec (meta, parent, n);
}


Word16Vec*
Word16Vec::create (Word16Vec* old, int newsize)
{
    void* loc = PALLOC1(sizeof(Word16Vec) + align(newsize*sizeof(Word16)), old);
    return NEW(loc) Word16Vec (old, newsize);
}


void
Word16Vec::reset (EMPTY)
{
...
Ob*
Word16Vec::subObject (int start, int n)
{
...
Word32Vec::Word32Vec (int n)
    : BinaryOb(sizeof(Word32Vec) + n*sizeof(Word32), CLASS_META(Word32Vec), CLASS_SBO(Word32Vec))
{
    while (n--) this->word(n) = 0;
    Word32Vec::updateCnt();
}
...
Word32Vec::Word32Vec (Word32Vec* old, int newsize)
    : BinaryOb(sizeof(Word32Vec) + newsize*sizeof(Word32), old->meta(), old->parent())
{
    int oldsize = old->numberOfWords();
    memcpy(&this->word(0), &old->word(0), min(oldsize, newsize)*sizeof(Word32));
    if (newsize > oldsize)
...
Word32Vec*
Word32Vec::create (Word32Vec* old, int newsize)
{
...
Ob*
Word32Vec::subObject (int start, int n)
{
