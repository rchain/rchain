...
void
VirtualMachine::installEnv (pOb newenv)
{
    ASSIGN(ctxt, env, newenv);
}


inline
void
VirtualMachine::installCode (Code*	cd,
			     int	relativePc)
{
    code = cd;
...
void
VirtualMachine::installCtxt (pCtxt new_ctxt)
{
    if (debugging_level)
	printf("*** new strand\n");
...
void
VirtualMachine::installMonitor (Monitor* monitor)
{
    if (debugging_level)
...
void
VirtualMachine::setup (Code* exprCode, Code* printerCode, Location tag)
{
...
int
VirtualMachine::traversePtrs (PSOb__PSOb f)
{
    int sum = 0;
...
int
VirtualMachine::traversePtrs (SI__PSOb f)
{
    int sum = 0;
...
void
VirtualMachine::traversePtrs (V__PSOb f)
{
    useIfPtr(code, f);
    useIfPtr(ctxt, f);
...
void
VirtualMachine::handleException (Ob* v, Instr instr, Location tag)
{
...
void
VirtualMachine::handleXmitUpcall (Instr instr, Location tag)
{
...
void
VirtualMachine::handleApplyPrimSuspend (Instr instr)
{
...
void
VirtualMachine::handleMissingBinding (pOb symbol, Location)
{
...
void
VirtualMachine::handleFormalsMismatch (Template* formals)
{
    PROTECT(formals);
    ctxt->prepare();
...
void
VirtualMachine::handleVirtualMachineError ()
{
    ctxt->vmError();
}
...
VirtualMachine::handleSignal ()
{
  ...
  if (sigvec & IO_MASK) {
	 ...
	 */
      ...
      retry_select:
	fd_set rfds = fds;
	FD_CLR(1, &rfds); FD_CLR(2, &rfds); //remove stdout & stderr
	fd_set wfds; FD_ZERO(&wfds);   //don't look for writes
	fd_set efds = fds;
...
pOb
VirtualMachine::unwindAndApplyPrim (Prim* prim)
{
    PROTECT(prim);
...
pOb
VirtualMachine::unwindAndDispatch ()
{
    Tuple* new_argvec = NIL;
    Tuple* suffix = (Tuple*) ctxt->arg(ctxt->nargs);
    if (ctxt->nargs != 0 || suffix != NIL) {
	if (!IS_A(suffix, Tuple))
	    return BASE(ctxt->trgt)->runtimeError(ctxt, "&rest value is not a tuple");
	new_argvec = Tuple::create (ctxt->nargs + suffix->numberOfElements(),
				    ctxt->nargs,
				    suffix);
	for (int i = ctxt->nargs; i--;)
...
void
VirtualMachine::evaluate (pOb expr)
{
    Code* cp = BASE(expr)->compileWrt(TopEnv, NIV);
    if (cp != INVALID) {
	setup(cp, printResult, ArgReg(0));
  ...
  nextop:
      ...
      case opApplyPrimTag | NextOn  | UnwindOn:
	{
	  ...
	  unsigned p_num = WORD_OP_e0_op0(ext);
	    ...
	    result = (OP_f5_unwind(instr)
		      ? unwindAndApplyPrim(prim)
      ...
      case opLookupToArg | 0xf:
	{
	    ...
	    pOb KONST val = BASE(BASE(ctxt->selfEnv)->meta())->lookupOBO(ctxt->selfEnv, key, ctxt);
	    if (val == UPCALL) {
		ctxt->pc = code->relativize(pc.absolute);
		goto doNextThread;
	    }
	 ...
	 */
	ASSIGN(ctxt->argvec, elem(OP_f7_op0(instr)), ctxt->env->slot(OP_f7_offset(instr)));
	goto nextop;
      ...
      case opXferLexToArg | IndirectOn  | 7:
	{
	    ...
	    pOb env = ctxt->env;
	    while (level--) env = BASE(env)->parent();
	    if (OP_f7_indirect(instr))
		env = ((Actor*)env)->extension;
	    ASSIGN(ctxt->argvec, elem(OP_f7_op0(instr)), env->slot(OP_f7_offset(instr)));
	    goto nextop;
	}


      case opXferLexToReg | IndirectOff | 0:
	ASSIGN(ctxt, reg(OP_f7_op0(instr)), ctxt->env->slot(OP_f7_offset(instr)));
	goto nextop;
	 ...
	 */
	ctxt->argvec->elem(OP_f1_op0(instr)) = ctxt->argvec->elem(OP_f1_op1(instr));
	goto nextop;
	...
	goto nextop;
      ...
      case opIndLitToArg:
	ASSIGN(ctxt->argvec, elem(OP_f1_op0(instr)), code->lit(OP_f1_op1(instr)));
	goto nextop;
...
void
VirtualMachine::scheduleStrand (pCtxt strand)
{
    strandPool->enq(strand);
}



int
VirtualMachine::addSignalHandler (int sig, SIG_PF fn, Ob* ob)
{
...
int
VirtualMachine::deleteSignalHandler (int sig)
{
...
Ob*
VirtualMachine::initiateRosetteSignal (int sig)
{
...
void
VirtualMachine::addIoHandler (int fd, IO_HANDLER* fn, void* ob, int rbl)
{
    /*
...
void
VirtualMachine::deleteIoHandler (int fd)
{
    if ((fd < 0) || (fd >= nfds))
      return;
...
void
VirtualMachine::enableIo (int fd)
{
    FD_SET(fd, &fds);
}
...
void
VirtualMachine::disableIo (int fd)
{
    FD_CLR(fd, &fds);
}
...
Ob*
VirtualMachine::initiateRosetteIo (int fd)
{
...
void
VirtualMachine::printStats (FILE* f)
{
    systemMonitor->printStats(f);
}



pCtxt
makePrinterCtxt (Code* cp)
{
...
int
GLOBAL_OFFSET (char* name)
{
    Location loc = GlobalEnv->lex(SYMBOL(name), 0);
    return (GET_GENERIC_TYPE(loc) != LT_LexVariable || GET_LEXVAR_LEVEL(loc) != 0
	    ? -1
