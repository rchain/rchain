...
ObQue::resize (int newSize)
{
  ...
  if (tail < head) {
    ...
    memcpy ((char*)newArray, (char*)tail, offset*sizeof(Ob*));
    memcpy ((char*)(newArray+offset), (char*)array, (head-array)*sizeof(Ob*));
...
int
ObQue::traversePtrs (PSOb__PSOb f)
{
  if (empty())
    return 0;
  else {
