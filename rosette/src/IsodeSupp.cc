...
extern "C" {
  ...
  int RoInvokeRequest(int, int, int, PE, int, IP, int,
  ...
  int RyAssocRequest(OID, AEI, struct PSAPaddr*, struct PSAPctxlist*,
		     struct SSAPref*, struct RoSAPindication*, IFP, int,
		     Ob*, R_VFP);
  ...
  int AcAssocResponse(int, int, int, OID,
		      AEI, struct PSAPaddr*, struct PSAPctxlist*, int,
		      int, int, long, int, struct SSAPref*,
		      PE*, int, struct AcSAPindication*);
...
int Ro_Encode_and_Invoke (int fd, int op, int invokeID,
			  HFP efn, caddr_t in, struct RoSAPindication *roi)
{
  PE pe = NULLPE;
  int result;
 ...
 char* mypci, Ob* ob, int port)
{
  ...
  */
  ...
  return RyAssocRequest
    (ctx, aei, pa, pc, sf, roi, RoPService, NULL, ob,
...
extern "C" {
int AcAccept
(struct AcSAPstart* acs, struct SSAPref* sf, HFP svc,
 Ob* ob, R_VFP hndlr, struct AcSAPindication* aci, struct RoSAPindication* roi)
{
  int r,
      sd = acs -> acs_sd;
  struct PSAPstart* ps = &(acs -> acs_start);
  
  r = AcAssocResponse (sd, ACS_ACCEPT, ACS_USER_NULL, NULLOID,
		       NULLAEI, NULLPA, NULLPC, PC_ACCEPT,
 ...
 Ob* ob, struct AcSAPindication* aci, struct PSAPindication* pi)
{
  ...
  if (PSetIndications (sd, (HFP)RpsDATAser, (HFP)RpsTOKENser, (HFP)RpsSYNCser,
		       ...
		       (HFP)RpsFINISHser, (HFP)RpsABORTser,
		       pi)
      == NOTOK)
    return NOTOK;
