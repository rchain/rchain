...
RBLstring::RBLstring (int n, char c)
    : ByteVec(sizeof(RBLstring) + align(n), CLASS_META(RBLstring), CLASS_SBO(RBLstring), n)
{
    if (c == 0)
	memset(&byte(0), c, n);
    else {
...
Ob*
RBLstring::indexedSize ()
{
    return FIXNUM(numberOfBytes()-1);
}
...
RBLstring::nth (int n)
{
    return RBLCHAR(byte(n));
}


Ob*
RBLstring::setNth (int n, Ob* v)
{
    byte(n) = CHARVAL(v);
    return this;
}
...
Ob*
RBLstring::subObject (int start, int n)
{
...
int
stringMemQAux(RBLstring* string, char c)
{
...
DEF("string-split",stringSplit, 2, 3)
{
  ...
  PROTECT(string); PROTECT(separators); PROTECT(__CTXT__);
  ...
  if (nsep == 0) {
    ...
    for (x = 0; ((i < lim) && (x < sz));) {
      for (; x < sz; x++)
	if (!stringMemQAux(separators, string->byte(x))) break;
      for (y = x; y < sz; y++)
	if (stringMemQAux(separators, string->byte(y)))  break;
      if (y > x) {
	i++;
	x = y;
      }
