...
int
slashify_char (char c, char buf[], int slash_blank)
{
...
Word32* mem_set_field (Word32* addr, int offset, int span, Word32 bits)
{
  ...
  switch (span) {
  case 8: 
    * (unsigned char*)((char*)addr + (offset/BITS(char))) = (unsigned char)bits;
    break;
  case 16:
    * (unsigned short*)((short*)addr + (offset/BITS(short))) = (unsigned short)bits;
    break;
  case 32:
    * (unsigned long*)((long*)addr + (offset/BITS(long))) = (unsigned long)bits;
    break;
  default:
    {
    /*
...
#ifndef HPUX
  if ((flags = fcntl(fd, F_GETFL, 0)) == -1) return -errno;
  if (fcntl (fd, F_SETFL, flags | FCNTL_NONBLOCK) == -1) return -errno;
#else
  flags = desiredState;
  if (ioctl(fd, FIOSNBIO, &flags) == -1) return -errno;
  flags = desiredState;
  if (ioctl(fd, FIOASYNC, &flags) == -1) return -errno;
#endif

  return fd;
}

void deleteIoHandler (int fd)
{
...
pOb
Ob::primitiveInitialize (pCtxt ctxt)
{
  int n = ctxt->nargs - 1;
...
pOb
RblAtom::primitiveInitialize (pCtxt ctxt)
{
  int n = ctxt->nargs - 1;
...
pOb
Actor::primitiveInitialize (pCtxt ctxt)
{
  PROTECT_THIS(Actor);
  PROTECT(ctxt);
  int n = ctxt->nargs - 1;
...
DEF("memcpy",unix_memcpy, 3, 3)
{
  ...
  if ((dest_addr >= local_page_size) &&
    ...
    return ADDR_TO_FIXNUM(memcpy((char*)dest_addr, (char*)src_addr, n_bytes));
  else
    PRIM_ERROR("invalid address");
}
...
inline void
setup_io(int in, int out, int err)
{
...
{
  struct C_C { char f1; char f2; } c_c;
  struct C_S { char f1; short f2; } c_s;
  struct C_L { char f1; long f2; } c_l;
...
DEF("tuple-unzip",tupleUnZip, 1, 2)
{
  ...
  PROTECT(tpl); PROTECT(__CTXT__);
  Tuple* ans = Tuple::create(n, NIV);
  PROTECT(ans);
  int sdn = sz / n; int smn = sz % n;

  Tuple* tmp;

  for (; i < n; i++) {
    if (i < smn) j = sdn+1; else j = sdn;
...
{
  int sz = 0; int i = 0; int j = 0; int n = NARGS; int x = 0; int mx = 0;
  for (; i < n; i++) {
    if (IS_A(ARG(i), Tuple)) {
