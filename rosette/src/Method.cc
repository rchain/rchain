...
StdMthd*
StdMthd::create (Code* code, Ob* id, Ob* source)
{
...
StdMthd::dispatch (Ctxt* ctxt)
{
     ...
     */
    PROTECT_THIS(StdMthd); PROTECT(ctxt);
...
StdMthd::invoke (Ctxt* ctxt)
{
    ...
    ctxt->rslt = NIV;
    ctxt->pc = 0;
    CHECK_STORE(ctxt, (VAL(ctxt->arg(0)), VAL(surrogate), VAL(SELF->code), VAL(ctxt)));
    ctxt->scheduleStrand();
    return SUSPENDED;
}
...
ReflectiveMthd::ReflectiveMthd (Code* code, Ob* id, Ob* source)
    : StdMthd(sizeof(ReflectiveMthd), CLASS_META(ReflectiveMthd), CLASS_SBO(ReflectiveMthd),
	      code, id, source)
{
...
ReflectiveMthd*
ReflectiveMthd::create (Code* code, Ob* id, Ob* source)
{
