...
/*
 * $Header: /mcc/project/carnot/root/master/pub-ess/src/rbl-rosy.cc,v 1.1.1.1 1993/02/12 01:25:50 tomlic Exp $
 *
 * $Log: rbl-rosy.cc,v $
...
#ifndef __RCS_ID__
#define __RCS_ID__
static const char *rcsid = "$Header: /mcc/project/carnot/root/master/pub-ess/src/rbl-rosy.cc,v 1.1.1.1 1993/02/12 01:25:50 tomlic Exp $";
#endif
...
### START insert of Isode-support.cc ####
extern StdOprn* oprnResumeIO;
extern Prim* obRuntimeError;

inline void
AddIsodeIoHandler (int fd, IO_HANDLER* handler)
{
  vm->addIoHandler (fd, handler, INVALID, 1);
}

void
SetIoPool (int fd, pOb ob)
{
  vm -> ioPool[fd] = ob;
}
...
pOb
ROI2ByteVec (struct ROI* indication, pTuple protos)
{
...
void
IndicationToRosette (int fd, struct ROI* indication)
{
...
int
RPsHandler (int fd, int type, char* ind)
{
  pTuple tpl	= (pTuple) vm -> ioPool[fd];
  pOb ob	= tpl -> nth (0);
  pTuple protos = (pTuple) tpl -> nth (1);
...
void
AcceptError2Rosette (pTuple tpl, int fd, char* err)
{
  pOb acter = tpl -> nth (0);
...
void
Disconnect2Rosette (pTuple tpl, int fd, char* td, int td_sz)
{
  pOb acter = tpl -> nth (0);
  pTuple oprns = (pTuple) tpl -> nth (1);
...
void
IsodeAccept2Rosette (pTuple tpl, int fd, char* acs, int acs_sz)
{
  pOb acter = tpl -> nth (0);
  pTuple oprns = (pTuple) tpl -> nth (1);
...
void*
fdopenOstream (int fd, char* mode)
{
...
void
ConnectEventToRosette (int type, int fd, Ob* ob)
{
...
void
AcceptEventToRosette (int type, int fd, Ob* ob)
{
  PROTECT (ob);
  int more;
...
void
TcpEventToRosette (int type, int fd, Ob* ob)
{
  int r, more;
...
void
StringEventToRosette (int type, int fd, Ob* ob)
{
  int r, more;
...
inline void deleteIoHandler (int fd)
{
  vm -> deleteIoHandler(fd);
}

### END insert of Isode-support.cc ###
...
int Ro_Encode_and_Invoke (int fd, int op, int invokeID,
			  IFP efn, caddr_t in, struct RoSAPindication *roi)
{
  PE pe = NULLPE;
  int result;
  ...
  PY_pepy[0] = 0;
  if (! efn ||
      (result = (*efn) (&pe, 1, NULL, NULLCP, in)) == OK)
    result = RoInvokeRequest
      (fd, op, ROS_ASYNC, pe, invokeID, NULLIP, ROS_NOPRIO, roi);
  else
    result = rosaplose
      (roi, ROS_CONGEST, NULLCP,
...
int Assoc_Initiate
(char* myname, char* hostname, char* myservice, char* mycontext,
 char* mypci, Ob* ob, int port)
{
  int	iloop,
  	sd;
  char	buffer[BUFSIZ],
  ...
  if (port)
    (pa -> pa_addr).sa_addr.ta_addrs[0].na_un.un_na_tcp.na_tcp_port
      = port;
  ...
  return RyAssocRequest
    (ctx, aei, pa, pc, sf, roi, RoPService, NULL, ob, IndicationToRosette);
}

int RyAssocRequest
(OID ctx, AEI aei, struct PSAPaddr* pa, struct PSAPctxlist* pc,
 struct SSAPref* sf, struct RoSAPindication* roi, IFP svc, int asy,
 Ob* ob, IFP hndlr)
{
  struct AcSAPconnect accs;
  ...
  if (AcAsynAssocRequest (ctx, NULLAEI, aei, NULLPA, pa, pc, NULLOID,
		      0, ROS_MYREQUIRE, SERIAL_NONE, 0, sf, NULLPEP,
		      0, NULLQOS, acc, aci, asy)
      == NOTOK) {
...
int PsAssocRequest
(OID ctx, AEI aei, struct PSAPaddr* pa, struct PSAPctxlist* pc,
 struct SSAPref* sf, struct PSAPindication* pi, int asy, Ob* ob)
{
  struct AcSAPconnect accs;
  struct AcSAPconnect *acc = &accs;
  struct AcSAPindication acis;
  ...
  if (AcAsynAssocRequest (ctx, NULLAEI, aei, NULLPA, pa, pc, NULLOID,
    ...
    if (acc -> acc_result != ACS_ACCEPT) {
      ...
      if (PSetIndications (sd, RpsDATAser, RpsTOKENser, RpsSYNCser,
			       RpsACTIVITYser, RpsREPORTser,
			       RpsFINISHser, RpsABORTser,
			   pi)
	  == NOTOK)
	return NOTOK;
...
void RpsDATAfree (struct PSAPdata* px)
{
  PXFREE (px);
}
...
int AcAccept
(struct AcSAPstart* acs, struct SSAPref* sf, int svc,
 Ob* ob, IFP hndlr, struct AcSAPindication* aci, struct RoSAPindication* roi)
{
  int r,
      sd = acs -> acs_sd;
  struct PSAPstart* ps = &(acs -> acs_start);
  
  r = AcAssocResponse (sd, ACS_ACCEPT, ACS_USER_NULL,
		       NULLOID, NULLAEI, NULLPA, NULLPC, PC_ACCEPT,
		       ps -> ps_prequirements, SR_DUPLEX, SERIAL_NONE,
		       ps -> ps_settings, sf, NULLPEP, 0, aci);
  if (r == NOTOK)
    return NOTOK;
...
int PsAcAccept
(struct AcSAPstart* acs, struct SSAPref* sf,
 Ob* ob, struct AcSAPindication* aci, struct PSAPindication* pi)
{
  int r,
      sd = acs -> acs_sd;
  struct PSAPstart* ps = &(acs -> acs_start);
...
int AssocRelRequest
(int sd, struct AcSAPrelease* acr, struct AcSAPindication* aci)
{
  int result;
...
int AssocRelResponse
(int sd, int status, int reason, struct AcSAPindication* aci)
{
  int result;
