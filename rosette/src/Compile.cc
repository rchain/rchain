...
AttrNode::AttrNode (int sz, bool valueContext)
    : BinaryOb(sz, CLASS_META(AttrNode), CLASS_SBO(AttrNode)), dest(LocRslt),
      av_size(0),
      outstanding(0),
      word(0),
...
int
AttrNode::traversePtrs (PSOb__PSOb f)
{
...
int
AttrNode::traversePtrs (SI__PSOb f)
{
    int sum = BinaryOb::traversePtrs(f);
    pOb* p = &slot(SLOT_NUM(AttrNode, cu));
    KONST pOb* end = endp();
...
void
AttrNode::traversePtrs (V__PSOb f)
{
...
inline
void
AttrNode::emitF0 (Opcode opcode, unsigned op)
{
...
inline
void
AttrNode::emitF1 (Opcode opcode, unsigned op0, unsigned op1)
{
...
inline
void
AttrNode::emitF2 (Opcode opcode, unsigned op0, unsigned op1)
{
...
inline
void
AttrNode::emitF3 (Opcode opcode, unsigned op0, unsigned op1, unsigned op2)
{
...
inline
void
AttrNode::emitF4 (Opcode opcode, unsigned unwind, unsigned next, unsigned nargs, unsigned op0)
{
...
inline
void
AttrNode::emitF5 (Opcode opcode, unsigned unwind, unsigned next, unsigned op0)
{
...
inline
void
AttrNode::emitF6 (Opcode opcode, unsigned pc)
{
...
inline
void
AttrNode::emitF7 (Opcode opcode, unsigned indirect, unsigned level, unsigned offset, unsigned op0)
{
...
inline
void
AttrNode::emitE0 (unsigned op0, unsigned op1)
{
...
inline
void
AttrNode::emitE1 (unsigned op0)
{
    cu->codebuf->emitE1(op0);
}

inline
void
AttrNode::emitE2 (unsigned op0, unsigned op1)
{
    cu->codebuf->emitE2(op0, op1);
}

void
AttrNode::changeDest (Location& newloc)
{
    dest = newloc;
}
...
void
AttrNode::emitDispatchCode (bool, bool, RtnCode, Label)
{
    NI("emitDispatchCode");
}
...
void
AttrNode::emitResumeCode (RtnCode rtn)
{ }



unsigned
AttrNode::getDestOffset ()
{
    return cu->extendLitvec(dest.atom);
}


void
AttrNode::emitAlloc (unsigned n)
{
    if (n == 0)
	return;
    else if (n < 256)
...
void
AttrNode::emitApplyPrim (unsigned primnum,
			 unsigned nargs,
			 bool     unwind,
			 RtnCode  rtn,
			 Label	  next)
...
void
AttrNode::emitExtend (Template* templat)
{
    PROTECT_THIS(AttrNode);
...
void
AttrNode::emitLookup (pOb symbol)
{
    KONST LocationType locType = (LocationType) GET_GENERIC_TYPE(dest);
    KONST int argno = GET_ARGREG_INDEX(dest);
    KONST int regno = GET_CTXTREG_INDEX(dest);
...
void
AttrNode::emitOpAndLabel (Opcode op,
			  Label  label)
{
    PROTECT_THIS(AttrNode);
...
void
AttrNode::emitRtn (RtnCode rtn, Label next)
{
    switch (rtn) {
...
void
AttrNode::emitImplicitRtn (Label next)
{
...
void
AttrNode::emitTaggedRtn (Label next)
{
    PROTECT_THIS(AttrNode);
...
void
AttrNode::emitUntaggedRtn (Label next)
{
    if (dest != LocLimbo)
...
void
AttrNode::emitXfer (Location source)
{
    if (source == dest)
	return;
    ...
    else if (sourceType == LT_LexVariable
	     && GET_LEXVAR_LEVEL(source) < 8
	...
	switch (destType) {
	  case LT_CtxtRegister:
	    SELF->emitF7(opXferLexToReg, ind, level, offset, GET_CTXTREG_INDEX(SELF->dest));
	    return;
	  case LT_ArgRegister:
	    if (GET_ARGREG_INDEX(dest) < 16) {
		SELF->emitF7(opXferLexToArg, ind, level, offset, GET_ARGREG_INDEX(SELF->dest));
		return;
	    }
	    else {
	...
	if (GET_ARGREG_INDEX(source) < 16 && GET_ARGREG_INDEX(SELF->dest) < 16) {
	    emitF1(opXferArgToArg, GET_ARGREG_INDEX(SELF->dest), GET_ARGREG_INDEX(source));
	    return;
	}
	else {
    ...
    else if (destType == LT_CtxtRegister && GET_CTXTREG_INDEX(SELF->dest) == CRN_Rslt) {
	Opcode opcode;
	unsigned offset;
	switch (sourceType) {
	  case LT_ArgRegister:
...
ConstNode::ConstNode (pOb  lit,
		      bool valueCtxt)
    : AttrNode(sizeof(ConstNode), valueCtxt),
      val(lit)
{
    ConstNode::updateCnt();
}
...
void
ConstNode::initialize (pOb		ctEnv,
		       pOb		freeEnv,
		       Location		dest,
		       CompilationUnit*	cu)
{
...
void
ConstNode::emitDispatchCode (bool    ctxtAvailable,
			     bool,
			     RtnCode rtn,
			     Label   next)
{
...
SymbolNode::SymbolNode (pOb  symbol,
			bool valueCtxt)
    : AttrNode(sizeof(SymbolNode), valueCtxt),
      loc(LocLimbo),
      sym(symbol)
{
...
void
SymbolNode::initialize (pOb              ctEnv,
			pOb              freeEnv,
			Location         dest,
			CompilationUnit* cu)
{
...
void
SymbolNode::emitDispatchCode (bool    ctxtAvailable,
			      bool,
			      RtnCode rtn,
			      Label   next)
{
...
FreeNode::FreeNode (FreeExpr* expr,
		    bool      valueCtxt)
...
void
FreeNode::initialize (pOb              ctEnv,
		      pOb              freeEnv,
		      Location         dest,
		      CompilationUnit* cu)
{
    PROTECT_THIS(FreeNode); PROTECT(ctEnv); PROTECT(freeEnv);
    ...
    AttrNode* node = BASE(SELF->expr->body)->makeAttrNode(GET_ATTR(*SELF,f_valueContext));
    ...
    SET_ATTR(*SELF,f_inlineableNode, GET_ATTR(*(SELF->body),f_inlineableNode));
...
void
FreeNode::changeDest (Location& new_loc)
{
    body->changeDest(new_loc);
}


void
FreeNode::emitDispatchCode (bool    ctxtAvailable,
			    bool    argvecAvailable,
			    RtnCode rtn,
			    Label   next)
{
...
void
FreeNode::emitResumeCode (RtnCode rtn)
{
    body->emitResumeCode(rtn);
}
...
FreeNode::primNumber ()
{
    return body->primNumber();
}



NullNode::NullNode (bool valueCtxt)
    : AttrNode(sizeof(NullNode), valueCtxt)
{
...
NullNode*
NullNode::create (bool valueCtxt)
{
...
void
NullNode::emitDispatchCode (bool,
			    bool,
			    RtnCode,
			    Label   next)
{
...
XferNode::XferNode (int source)
    : AttrNode(sizeof(XferNode), TRUE),
      source(FIXNUM(source))
{
    XferNode::updateCnt();
}
...
void
XferNode::emitDispatchCode (bool,
			    bool,
			    RtnCode,
			    Label   next)
{
...
inline
ExprStack::ExprStack ()
    : exprs(NIL), top(FIXNUM(0))
{ }


inline
CompoundNode::CompoundNode (int  sz, bool ctxt)
    : AttrNode(sz, ctxt)
{ }


void
CompoundNode::addTo (ExprStack*	exprstack,
		     AttrNode*	node)
{
    int n = FIXVAL(exprstack->top);
    if (n == 0) {
	PROTECT_THIS(CompoundNode); PROTECT(node);
...
void
CompoundNode::analyze (AttrNode* node)
{
...
void
CompoundNode::rearrangeInlinedExprs ()
{
...
int
CompoundNode::determineFree (ArgNum* free)
{
    int freeTop = 0;
...
void
CompoundNode::fixInlinedConflicts (ArgNum* free,
				   int freeTop)
{
    int N = FIXVAL(inlined.top);
	...
	if (GET_ARGREG_INDEX(ni->dest) <= highest_j) {
	    int tempReg = (freeTop > 0 && free[freeTop-1] > highest_j) ? free[--freeTop] : nextTemp++;
...
void
CompoundNode::emitPrefix (bool ctxtAvailable,
			  bool argvecAvailable)
{
...
void
CompoundNode::emitDispatchCode (bool	ctxtAvailable,
				bool	argvecAvailable,
				RtnCode	rtn,
				Label	exit)
{
...
void
CompoundNode::emitSimpleExprDispatchCode (RtnCode rtn,
					  Label   exit)
{
...
void
CompoundNode::emitInlinedExprDispatchCode (RtnCode rtn,
					   Label   exit)
{
...
void
CompoundNode::emitNestedExprDispatchCode (RtnCode	rtn,
					  Label		exit)
{
	...
	for (int i = 0; i < nexprs; i++) {
	    ...
	    AttrNode* node = (AttrNode*) exprs->elem(i);
	    node->emitDispatchCode(!CtxtAvailable, !ArgvecAvailable, rtn, nextExpr);
	    if (!last)
...
void
CompoundNode::emitNestedExprResumeCode (RtnCode rtn)
{
    int nexprs = FIXVAL(nested.top);
...
void
CompoundNode::emitResumeCode (RtnCode rtn)
{
    if (outstanding > 0) {
	PROTECT_THIS(CompoundNode);
...
void
CompoundNode::emitWrapup (RtnCode, Label)
{
    NI( "emitWrapup" );
}
...
BlockNode::BlockNode (BlockExpr* be,
		      bool       valueCtxt)
    : CompoundNode(sizeof(BlockNode), valueCtxt),
      expr(be)
{
    BlockNode::updateCnt();
}
...
void
BlockNode::initialize (pOb		ctEnv,
		       pOb		freeEnv,
		       Location		dest,
		       CompilationUnit*	cu)
{
    PROTECT_THIS(BlockNode); PROTECT(ctEnv); PROTECT(freeEnv);
    ...
    for (int i = 0; i < nexprs; i++) {
	AttrNode* node = BASE(SELF->expr->subExprs->elem(i))->makeAttrNode(FALSE);
	PROTECT(node);
	...
	switch (valueProducers) {
	  case 0: {
		  ...
		  const char* modifier = BOOLVAL(SELF->expr->implicit) ? "implicit " : "";
		  SELF->cu->warning("no value returned from %sblock in value-expecting position",
					 modifier);
	      }
		...
		default:
		  if (!GET_ATTR(*SELF,f_topLevel)) {
		      const char* modifier = BOOLVAL(SELF->expr->implicit) ? "an implicit" : "a";
		      SELF->cu->warning("more than one result may be returned from %s block expression",
					     modifier);
		break;
	    }
	}
...
void
BlockNode::changeDest (Location& newloc)
{
    int i = 0;
...
void
BlockNode::emitDispatchCode (bool	ctxtAvailable,
			     bool	argvecAvailable,
			     RtnCode	rtn,
			     Label	exit)
{
...
void
BlockNode::emitNestedExprDispatchCode (RtnCode	rtn, Label)
{
    Tuple* exprs = nested.exprs;
    int nexprs = FIXVAL(nested.top);
...
void
BlockNode::emitResumeCode (RtnCode rtn)
{
    int nexprs = FIXVAL(nested.top);
    if (nexprs > 0) {
	PROTECT_THIS(BlockNode);
...
RequestNode::RequestNode (int          sz,
			  RequestExpr* expr,
			  bool         valueCtxt)
...
RequestNode*
RequestNode::create (RequestExpr* expr,
		     bool         valueCtxt)
{
...
void
RequestNode::initialize (pOb			ctEnv,
			 pOb			freeEnv,
			 Location		dest,
			 CompilationUnit*	cu)
{
    ...
    PROTECT_THIS(RequestNode); PROTECT(node); PROTECT(ctEnv);
    PROTECT(freeEnv); PROTECT(cu);
    ...
    SET_ATTR(*SELF,f_inlineableNode, (SELF->outstanding == 0 && (SELF->primTrgt != FIXNUM(-1))));
    SET_ATTR(*SELF,f_producesValue, (SELF->dest != LocLimbo));
}


int
RequestNode::numberOfSubExprs ()
{
...
void
RequestNode::emitWrapup (RtnCode rtn,
			 Label   next)
{
...
void
RequestNode::emitXmit (unsigned nargs,
		       bool	unwind,
		       RtnCode  rtn,
		       Label    next)
{
...
TupleNode::TupleNode (TupleExpr* te,
		      bool       valueCtxt)
    : CompoundNode(sizeof(TupleNode), valueCtxt),
      expr(te)
{
    TupleNode::updateCnt();
}
...
void
TupleNode::initialize (pOb		ctEnv,
		       pOb		freeEnv,
		       Location		dest,
		       CompilationUnit*	cu)
{
    ...
    AttrNode* node = (AttrNode*) INVALID;
    PROTECT_THIS(TupleNode); PROTECT(node); PROTECT(ctEnv); PROTECT(freeEnv);
...
void
TupleNode::emitDispatchCode (bool	ctxtAvailable,
			     bool,
			     RtnCode	rtn,
			     Label	exit)
{
...
void
TupleNode::emitWrapup (RtnCode	rtn,
		       Label	next)
{
    int nelems = expr->numberOfElements();
	...
	if (av_size > nelems) {
	    ...
	    SELF->emitLit(NIL);
	    SELF->dest = temp;
	    SELF->emitApplyPrim(tplConsStar->primNumber(), nelems+1, FALSE, rtn, next);
	}
	else {
	    if (rtn == TaggedRtn)
		SELF->dest = LocRslt;
...
void
IfNode::initialize (pOb			ctEnv,
		    pOb			freeEnv,
		    Location		dest,
		    CompilationUnit*	cu)
{
    AttrNode* node = (AttrNode*) INVALID;
    PROTECT_THIS(IfNode); PROTECT(node); PROTECT(ctEnv); PROTECT(freeEnv);
    ...
    node = BASE(SELF->expr->trueBranch)->makeAttrNode(GET_ATTR(*SELF,f_valueContext));
    ASSIGN(SELF, trueNode, node);
    node->initialize(ctEnv, freeEnv, dest, SELF->cu);

    node = BASE(SELF->expr->falseBranch)->makeAttrNode(GET_ATTR(*SELF,f_valueContext));
    ASSIGN(SELF, falseNode, node);
    node->initialize(ctEnv, freeEnv, dest, SELF->cu);

    SET_ATTR(*SELF,f_inlineableNode,(condInlineable
	&& GET_ATTR(*(SELF->trueNode),f_inlineableNode)
	    && GET_ATTR(*(SELF->falseNode),f_inlineableNode)));

    if (condInlineable)
	SELF->av_size = max(SELF->av_size,
			    max(SELF->trueNode->av_size,
				SELF->falseNode->av_size));

    SET_ATTR(*SELF,f_producesValue, GET_ATTR(*(SELF->trueNode),f_producesValue));
...
void
IfNode::changeDest (Location& newloc)
{
...
void
IfNode::emitResumeCode (RtnCode rtn)
{
    PROTECT_THIS(IfNode);
		     ...
		     CompilationUnit*	cu)
{
    ...
    AttrNode* node = (AttrNode*) INVALID;
    PROTECT_THIS(LetNode); PROTECT(node); PROTECT(ctEnv); PROTECT(freeEnv);
    ...
    node = BASE(SELF->expr->body)->makeAttrNode(GET_ATTR(*SELF,f_valueContext));
...
void
LetNode::changeDest (Location& newloc)
{
    AttrNode::changeDest(newloc);
...
int
LetNode::numberOfSubExprs ()
{
    return expr->bindings->numberOfElements();
}


void
LetNode::emitResumeCode (RtnCode rtn)
{
...
void
LetNode::emitWrapup (RtnCode	rtn,
		     Label	next)
{
    PROTECT_THIS(LetNode);
...
LetrecNode::LetrecNode (LetrecExpr*	le,
			bool		valueCtxt)
...
LetrecNode*
LetrecNode::create (LetrecExpr* le,
		    bool        valueCtxt)
{
...
void
LetrecNode::emitDispatchCode (bool	ctxtAvailable,
			      bool	argvecAvailable,
			      RtnCode	rtn,
			      Label	next)
{
    ...
    SELF->emitExtend(SELF->templat);
    SELF->CompoundNode::emitDispatchCode(CtxtAvailable, !ArgvecAvailable, rtn, next);
}


void
LetrecNode::emitWrapup (RtnCode	rtn,
			Label	next)
{
...
MethodNode::MethodNode (MethodExpr* me,
			bool        valueCtxt)
    : AttrNode(sizeof(MethodNode), valueCtxt),
      expr(me),
      code((Code*) INVALID)
{
...
MethodNode*
MethodNode::create (MethodExpr* me,
		    bool        valueCtxt)
{
...
void
MethodNode::initialize (pOb			ctEnv,
			pOb			freeEnv,
			Location		dest,
			CompilationUnit*	cu)
{
...
void
MethodNode::emitDispatchCode (bool	ctxtAvailable,
			      bool	argvecAvailable,
			      RtnCode	rtn,
			      Label	next)
{
...
Code*
MethodNode::compileBody (pOb	ctEnv,
			 pOb	freeEnv)
{
    PROTECT_THIS(MethodNode); PROTECT(ctEnv); PROTECT(freeEnv);
    Template* templat = SELF->adjustFormals();
    PROTECT(templat);
    CompilationUnit* bodycu = CompilationUnit::create (SELF->expr->body,
						       SELF->cu->info,
						       SELF->expr);
...
ReflectiveMethodNode*
ReflectiveMethodNode::create (ReflectiveMethodExpr* rme,
			      bool                  valueCtxt)
{
...
Template*
ReflectiveMethodNode::adjustFormals ()
{
...
int
ReflectiveMethodNode::constructor ()
{
...
void
ProcNode::emitDispatchCode (bool	ctxtAvailable,
			    bool	argvecAvailable,
			    RtnCode	rtn,
			    Label	next)
{
...
SeqNode::SeqNode (AttrNode* first,
		  AttrNode* second,
		  bool      valueCtxt)
    : CompoundNode(sizeof(SeqNode), valueCtxt),
      first(first),
      second(second)
{
    SeqNode::updateCnt();
...
void
SeqNode::initialize (pOb		ctEnv,
		     pOb		freeEnv,
		     Location		dest,
		     CompilationUnit*	cu)
{
    PROTECT_THIS(SeqNode); PROTECT(ctEnv); PROTECT(freeEnv);
...
void
SeqNode::changeDest (Location& newloc)
{
    second->changeDest(newloc);
}


int
SeqNode::numberOfSubExprs ()
{
    return 1;
}
...
void
SeqNode::emitResumeCode (RtnCode rtn)
{
...
void
SeqNode::emitWrapup (RtnCode	rtn,
		     Label	next)
{
...
void
SetNode::initialize (pOb		ctEnv,
		     pOb		freeEnv,
		     Location		dest,
		     CompilationUnit*	cu)
{
    PROTECT_THIS(SetNode); PROTECT(ctEnv); PROTECT(freeEnv);
    ...
    SET_ATTR(*SELF,f_inlineableNode, GET_ATTR(*(SELF->valNode),f_inlineableNode));
...
void
SetNode::emitResumeCode (RtnCode rtn)
{
...
void
SetNode::emitWrapup (RtnCode	rtn,
		     Label	exit)
{
    PROTECT_THIS(SetNode);
    Location temp = dest;
...
void
GotoNode::emitDispatchCode (bool	ctxtAvailable,
			    bool	argvecAvailable,
			    RtnCode,
			    Label)
{
	...
	if (env == TopEnv)
	    ...
	    cu->abort("attempt to cut back more than %d lexical levels in jump to label '%s'",
		      MaximumCut,
		      SYMPTR(labelName));
...
LabelNode::LabelNode (LabelExpr* le,
		      bool       valueCtxt)
...
void
LabelNode::initialize (pOb		ctEnv,
		       pOb		freeEnv,
		       Location		dest,
		       CompilationUnit*	cu)
{
    PROTECT_THIS(LabelNode); PROTECT(ctEnv); PROTECT(freeEnv);
    ...
    SET_ATTR(*SELF,f_inlineableNode,GET_ATTR(*(SELF->bodyNode),f_inlineableNode));
    SET_ATTR(*SELF,f_simpleNode , GET_ATTR(*(SELF->bodyNode),f_simpleNode));
    SET_ATTR(*SELF,f_producesValue, GET_ATTR(*(SELF->bodyNode),f_producesValue));
}


void
LabelNode::changeDest (Location& newloc)
{
    bodyNode->changeDest(newloc);
}
...
void
LabelNode::emitDispatchCode (bool	ctxtAvailable,
			     bool	argvecAvailable,
			     RtnCode	rtn,
			     Label	next)
{
...
void
LabelNode::emitResumeCode (RtnCode rtn)
{
    bodyNode->emitResumeCode(rtn);
}


int
LabelNode::primNumber ()
{
    return bodyNode->primNumber();
}
...
CompilationUnit::CompilationUnit (pOb         info,
				  AttrNode*   graph,
				  CodeBuf*    codebuf,
				  Tuple*      litvec,
				  LabelTable* labels)
    : BinaryOb(sizeof(CompilationUnit),
	       CLASS_META(CompilationUnit),
...
CompilationUnit*
CompilationUnit::create (pOb expr,
			 pOb info,
			 pOb source)
{
    PROTECT(info); PROTECT(source);
    AttrNode* graph = BASE(expr)->makeAttrNode(TRUE);	PROTECT(graph);
    CodeBuf* codebuf = CodeBuf::create ();		PROTECT(codebuf);
    Tuple* litvec = Tuple::create (1, source);		PROTECT(litvec);
...
int
CompilationUnit::traversePtrs (PSOb__PSOb f)
{
    int sum = BinaryOb::traversePtrs(f);
    sum += useIfPtr(&info, f);
...
int
CompilationUnit::traversePtrs (SI__PSOb f)
{
    int sum = BinaryOb::traversePtrs(f);
    sum += useIfPtr(info, f);
    sum += useIfPtr(graph, f);
...
void
CompilationUnit::traversePtrs (V__PSOb f)
{
    BinaryOb::traversePtrs(f);
    useIfPtr(info, f);
    useIfPtr(graph, f);
...
unsigned
CompilationUnit::extendLitvec (pOb val)
{
...
Code*
CompilationUnit::compileExpr (pOb	ctEnv,
			      pOb	freeEnv)
{
    PROTECT_THIS(CompilationUnit); PROTECT(ctEnv); PROTECT(freeEnv);
    ...
    SELF->graph->emitDispatchCode(CtxtAvailable, !ArgvecAvailable, UntaggedRtn, NoneRemaining);
...
Code*
CompilationUnit::compileBody (Template*	templat,
			      pOb	ctEnv,
			      pOb	freeEnv)
{
    PROTECT_THIS(CompilationUnit); PROTECT(templat); PROTECT(ctEnv); PROTECT(freeEnv);
...
void
CompilationUnit::abort (const char* msg, ...)
{
    va_list args;
    va_start(args, msg);
...
void
CompilationUnit::warning (const char* msg, ...)
{
    va_list args;
    va_start(args, msg);
...
void
CompilationUnit::vwarning (const char* severity, const char* fmt, va_list args)
{
...
void
CompilationUnit::atTopLevel ()
{
    SET_ATTR(*graph,f_topLevel,TRUE);
}


Label
CompilationUnit::newLabel ()
{
...
void
CompilationUnit::setLabel (Label label)
{
...
void
CompilationUnit::registerLabel (Label label)
{
...
AttrNode*
BlockExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
RequestExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
SendExpr::makeAttrNode (bool valueCtxt)
{
    if (valueCtxt) {
	/*
...
AttrNode*
TupleExpr::makeAttrNode (bool valueCtxt)
{
    if (ConstantP())
	...
	if (!tailarg)
	    ...
	    for (int i = nelems; i--;) te->elem(i) = SELF->elem(i);
	...
	PROTECT_THIS(TupleExpr); PROTECT(te);
...
AttrNode*
IfExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
LetExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
LetrecExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
QuoteExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
MethodExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
ReflectiveMethodExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
FreeExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
ProcExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
SeqExpr::makeAttrNode (bool valueCtxt)
{
      ...
      default:
	{
	    ...
	    PROTECT_THIS(SeqExpr); PROTECT(tail);
	    tail = BASE(SELF->subExprs->elem(--nexprs))->makeAttrNode(valueCtxt);
	    while (nexprs--) {
		AttrNode* head = BASE(SELF->subExprs->elem(nexprs))->makeAttrNode(TRUE);
...
AttrNode*
SetExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
GotoExpr::makeAttrNode (bool valueCtxt)
{
...
AttrNode*
LabelExpr::makeAttrNode (bool valueCtxt)
{
