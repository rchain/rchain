...
trait HistoryActionsTests[F[_]]
  ...
  "consume then createCheckpoint" should "return the expected hash and the TrieStore should contain the expected value" in
    withTestSpace { (store, space) =>
      ...
      for {
        ...
        root       <- getRootHash(store, store.trieBranch)
        retrieved  <- space.retrieve(root, channelsHash)
        _          = retrieved shouldBe None
        checkpoint <- space.createCheckpoint()
        _          = checkpoint.root shouldBe nodeHash
        _ = history.lookup(store.trieStore, store.trieBranch, channelsHash) shouldBe Some(
          gnat
        )
  ...
  "consume twice then createCheckpoint" should "persist the expected values in the TrieStore" in
    withTestSpace { (store, space) =>
      ...
      val channelsHash2                 = store.hashChannels(gnat2.channels)
      ...
      for {
        ...
        _ = history.lookup(store.trieStore, store.trieBranch, channelsHash2) shouldBe Some(
          gnat2
        )
        root2      <- getRootHash(store, store.trieBranch)
    ...
    forAll { (data: TestConsumeMap) =>
      withTestSpace { (store, space) =>
        for {
          ...
          channelHashMap                      = gnats.map(gnat => store.hashChannels(gnat.channels) -> gnat).toMap
  ...
  "createCheckpoint, consume, createCheckpoint, reset to first checkpoint, reset to second checkpoint" should
    ...
    (store, space) =>
      for {
        ...
        contents1: Map[Seq[String], Row[Pattern, String, StringsCaptor]] = store.toMap
        _                                                                = store.isEmpty shouldBe false
      ...
      } yield (store.toMap shouldBe contents1)
  }
        ...
        val states = data.zipWithIndex.map {
          case (produces, chunkNo) =>
            for {
              ...
              (State(checkpoint.root, store.toMap, store.joinMap), chunkNo)
            }
        }
        for {
          stateEffect <- states.toList.sequence
          res         <- validateIndexedStates(store, space, stateEffect, "produces_reset")
        } yield res
  ...
  "when resetting to a bunch of checkpoints made with consumes, the store" should
    ...
    val prop = Prop.forAllNoShrink { (data: Seq[TestConsumeMap]) =>
      withTestSpace { (store, space) =>
        ...
        for {
          stateEffect <- states.toList.sequence
          res         <- validateIndexedStates(store, space, stateEffect, "consumes_reset")
        } yield res
      }
    }
    check(prop)
  ...
  "when resetting to a bunch of checkpoints made with consumes and produces, the store" should
    ...
    val prop = Prop.forAllNoShrink { (data: Seq[(TestConsumeMap, TestProduceMap)]) =>
      withTestSpace { (store, space) =>
        ...
        val states = data.zipWithIndex.map {
          case ((consumes, produces), chunkNo) =>
            for {
              ...
              (
                State(checkpoint.root, store.toMap, store.joinMap),
                chunkNo
              )
            }
        }
        for {
          stateEffect <- states.toList.sequence
          res         <- validateIndexedStates(store, space, stateEffect, "produces_consumes_reset")
        } yield res
      }
    }
    check(prop)
  ...
  "consume, produce, produce" should "result in the expected trace log" in withTestSpace {
    (store, space) =>
      val channels = List("ch1", "ch2")
  ...
  it should "not be persisted to the history trie after second install" in withTestSpace {
    (store, space) =>
      val key      = List("ch1")
      val patterns = List(Wildcard)
  ...
  it should "be available after resetting to a checkpoint" in withTestSpace { (store, space) =>
    val channel      = "ch1"
    val datum        = "datum1"
