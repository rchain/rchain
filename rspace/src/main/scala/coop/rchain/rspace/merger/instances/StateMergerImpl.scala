...
object StateMergerImpl {
  ...
  final case class HistoriesWithChanges[F[_], C, P, A, K](
      startState: HistoryRepository[F, C, P, A, K],
      endState: HistoryRepository[F, C, P, A, K],
      events: Seq[Event]
  )
  ...
  final case class StateMergerImpl[F[_]: Concurrent: Log, C, P, A, K](
    ...
    private def computeHistoryActions(
        ...
        mergings: Seq[HistoriesWithChanges[F, C, P, A, K]],
        joinMap: Ref[F, Map[Blake2b256Hash, Seq[Seq[C]]]]
    ): F[List[HistoryAction]] = {

      val events = mergings.flatMap(_.events)
      ...
      // TODO now all channels in all mergings are checked, reduce check to only channels inside partolular merging
      // streams for compute data and continuation changes
      val dataChangesComputes = changedChannels.produceChanges.map(
        ch =>
          fs2.Stream.eval(
            computeDataChanges(ch, mainStateReader, mergings)
          )
      )
      val contChangesComputes = changedChannels.consumeChanges.map(
        ch =>
          fs2.Stream
            .eval(
              computeContinuationChanges(ch, mainStateReader, mergings, joinMap)
            )
            .filter(_.isDefined)
            .map(_.get)
      )
    ...
    override def merge(
      ...
      for {
        ...
        historiesWithChanges <- toMerge.toList.traverse(
                                 m =>
                                   for {
                                     start  <- historyRepo.reset(m.startState)
                                     finish <- historyRepo.reset(m.endState)
                                   } yield HistoriesWithChanges(start, finish, m.events)
                               )

        historyActions <- Stopwatch.time(Log[F].info(_))(
                           s"compute historyActions: ${toMerge.flatMap(_.events).size} events, merged ${historiesWithChanges.size} states"
                         )(
    ...
    private def computeDataChanges(
        ...
        mergingStatesHistories: Seq[HistoriesWithChanges[F, C, P, A, K]]
        ...
        mergeResult <- mergingStatesHistories.toList.foldLeftM(dataAtMain)(
                        (acc, mergingStateHistory) =>
                          for {
                            ...
                            // is about 30% slower
                            dataAtBase <- mergingStateHistory.startState
                                           .getDataWithBytes(dataHash)
                                           .map(_.toVector)
                            dataAtMerge <- mergingStateHistory.endState
                                            .getDataWithBytes(dataHash)
                                            .map(_.toVector)
            ...
            mergeResult <- mergingStatesHistories.toList.foldLeftM(continuationsAtMain)(
                            (acc, mergingStateHistory) =>
                              for {
                                ...
                                // using `dataAtMerge diff dataAtBase` compared with `filterNot` using ByteVector key
                                // is about 30% slower
                                continuationsAtBase <- mergingStateHistory.startState
                                                        .getContinuationsWithBytes(consumeHash)
                                continuationsAtMerge <- mergingStateHistory.endState
                                                         .getContinuationsWithBytes(consumeHash)
