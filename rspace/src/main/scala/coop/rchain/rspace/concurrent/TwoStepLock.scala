...
trait TwoStepLock[K] {
  def acquire[R, S, W](keysA: Seq[K])(phaseTwo: () => Seq[K])(thunk: => W)(implicit o: Ordering[K]): W
}
...
class DefaultTwoStepLock[K] extends TwoStepLock[K] {
  ...
  override def acquire[R, S, W](keysA: Seq[K])(phaseTwo: () => Seq[K])(thunk: => W)(implicit o: Ordering[K]): W = {
