...
class ReplayRSpace[C, P, A, K](val store: IStore[C, P, A, K], val branch: Branch)(
  ...
  def consume(channels: Seq[C], patterns: Seq[P], continuation: K, persist: Boolean)(
    ...
    store.eventsCounter.registerConsume {
      ...
      store.withTxn(store.createTxnWrite()) { txn =>
        def runMatcher(maybeComm: Option[COMM]): Option[Seq[DataCandidate[C, A]]] = {
        ...
        def storeWaitingContinuation(replays: ReplayData, consume: Consume): None.type = {
          store.putWaitingContinuation(
            txn,
            channels,
            WaitingContinuation(patterns, continuation, persist, consume))
        ...
        def handleMatches(mats: Seq[DataCandidate[C, A]],
                          replays: ReplayData,
                          consumeRef: Consume,
                          comms: Multiset[COMM]): Option[(K, Seq[A])] = {
        ...
        replays.get(consumeRef) match {
          case None =>
            runMatcher(None) match {
              case None =>
                storeWaitingContinuation(replays, consumeRef)
              case Some(_) =>
  ...
  def install(channels: Seq[C], patterns: Seq[P], continuation: K)(
      implicit m: Match[P, A]): Option[(K, Seq[A])] = None
    ...
    store.eventsCounter.registerProduce {
      store.withTxn(store.createTxnWrite()) { txn =>
        ...
        def runMatcher(maybeComm: Option[COMM],
                       produceRef: Produce,
                       groupedChannels: Seq[Seq[C]]): Option[ProduceCandidate[C, P, A, K]] =
        ...
        def storeDatum(replays: ReplayData, produce: Produce): None.type = {
          store.putDatum(txn, Seq(channel), Datum(data, persist, produce))
        ...
        def handleMatch(mat: ProduceCandidate[C, P, A, K],
                        replays: ReplayData,
                        produceRef: Produce,
                        comms: Multiset[COMM]): Option[(K, Seq[A])] =
          mat match {
        ...
        replays.get(produceRef) match {
          ...
          case Some(comms) =>
            val maybeProduceCandidate: Option[ProduceCandidate[C, P, A, K]] =
              comms
                .iterator()
                .asScala
                .map { comm: COMM =>
                  runMatcher(Some(comm), produceRef, groupedChannels)
                }
                .collectFirst {
                  case Some(x) => x
                }

            maybeProduceCandidate match {
              case None =>
                storeDatum(replays, produceRef)
              case Some(produceCandidate) =>
...
object ReplayRSpace {
  ...
  def create[C, P, A, K](context: Context[C, P, A, K], branch: Branch)(
      ...
      sp: Serialize[P],
      sa: Serialize[A],
      sk: Serialize[K]): ReplayRSpace[C, P, A, K] = {
    ...
    new ReplayRSpace[C, P, A, K](mainStore, branch)
  }
}
