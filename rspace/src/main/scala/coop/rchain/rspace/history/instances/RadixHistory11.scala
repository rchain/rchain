...
import coop.rchain.rspace.history.RadixTree11._
...
/**
  ...
  */
object RadixHistory11 {
  def apply[F[_]: Sync: Parallel](
      rootHash: Blake2b256Hash,
      store: RadixStore3[F]
  ): F[RadixHistory11[F]] =
    for {
      impl     <- Sync[F].delay(new RadixTreeImpl11[F](store))
      rootNode <- impl.loadNode(rootHash.bytes.toArray, noAssert = true)
    } yield RadixHistory11(rootHash, rootNode, impl, store)
}

final case class RadixHistory11[F[_]: Sync: Parallel](
    rootHash: Blake2b256Hash,
    rootNode: Node,
    impl: RadixTreeImpl11[F],
    store: RadixStore3[F]
) extends History[F] {
