...
object RadixTree12 {
  ...
  object codecs {
    ...
    def encode(node: Node): ByteVector = {
      ...
      def loop(index: Int, size: Int): Int =
        if (sizeNode > index)
          node(index) match {
            case EmptyChild => loop(index + 1, size)
            ...
            case Leaf(leafPrefix, value, varLength) =>
              ...
              loop(index + 1, size + 2 + sizePrefix + +sizeLength + sizeValue)
            ...
            case NodePtr(ptrPrefix, ptr) =>
              ...
              loop(index + 1, size + 2 + sizePrefix + sizePtr)
          } else size
    ...
    def decode(data: ByteVector): Node = {
      import scala.collection.mutable.ListBuffer

      val arr: Array[Byte] = data.toArray
        ...
        try {
          ...
          if ((secondByte & 0x80) == 0x00) {   //Leaf
            if ((secondByte & 0x40) != 0x00) { //leafValue have variable length
              val leafValueSize: Int = byteToInt(buf.get())
...
//    def decode2(data: ByteVector): Node = {
//      import scala.collection.mutable.ListBuffer
//
//      val arr: Array[Byte] = data.toArray
//
//      def loop(index: Int, node: Node): Node = {}
//
//      val buf = ByteBuffer.allocate(arr.length)
//      buf.put(arr)
//      buf.rewind
...
//    }
  ...
  def hashNode(node: Node): (ByteVector, ByteVector) = {
    ...
    (ByteVector(Blake2b256.hash(bytes.toArray)), bytes)
  }

  def byteToInt(b: Byte): Int = b & 0xff

  class RadixTreeImpl12[F[_]: Sync: Parallel](store: RadixStore4[F]) {
