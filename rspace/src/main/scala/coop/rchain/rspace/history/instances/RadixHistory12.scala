...
import coop.rchain.rspace.history.RadixTree12._
...
/**
  ...
  */
object RadixHistory12 {
  def apply[F[_]: Sync: Parallel](
      rootHash: Blake2b256Hash,
      store: RadixStore4[F]
  ): F[RadixHistory12[F]] =
    for {
      impl     <- Sync[F].delay(new RadixTreeImpl12[F](store))
      rootNode <- impl.loadNode(rootHash.bytes, noAssert = true)
    } yield RadixHistory12(rootHash, rootNode, impl, store)
}

final case class RadixHistory12[F[_]: Sync: Parallel](
    rootHash: Blake2b256Hash,
    rootNode: Node,
    impl: RadixTreeImpl12[F],
    store: RadixStore4[F]
) extends History[F] {
