...
import coop.rchain.shared.ByteArrayOps.RichByteArray
...
//  sealed trait PrefixCollection {
//    def head: Byte
//    def tail: PrefixCollection
//  }
  ...
  final case class NodePtr(prefix: ByteVector, ptr: ArrB)                          extends Child
      ...
      while (buf.position() < buf.capacity()) {
        try {
          ...
          if ((secondByte & 0x80) == 0x00) {   //Leaf
            ...
            buf.get(ptr)
            node += NodePtr(ByteVector(prefix), ptr)
          }
        } catch {
          case _: Exception =>
  ...
  /**
    * Compare 2 nodes by content.
    * Return true - if nodes same, false - in if they are not equal.
    */
  def equalityOfNodes(node1: Node, node2: Node): Boolean = //todo very slow operations (like serialization)
    (node1 zip node2).forall {
      case (EmptyChild, EmptyChild) => true
      case (Leaf(leafPrefix1, leafValue1, flag1), Leaf(leafPrefix2, leafValue2, flag2)) =>
        if (leafPrefix1 == leafPrefix2 && leafValue1 == leafValue2 && flag1 == flag2) true
        else false
      case (NodePtr(ptrPrefix1, ptr1), NodePtr(ptrPrefix2, ptr2)) =>
        if (ptrPrefix1 == ptrPrefix2 && ptr1.deep == ptr2.deep) true
        else false
      case (_, _) => false
    }
  ...
  /**
    ...
    */
  def hashNode(node: Node): (ArrB, ByteBuffer) = {
    import coop.rchain.crypto.hash.Blake2b256
    val buf = codecs.encode(node)
    val arr = new ArrB(buf.remaining())
    buf.get(arr)
  ...
  class RadixTreeImpl11[F[_]: Sync: Parallel](store: RadixStore3[F]) {

    /**
      * Load and decode serializing data from KVDB.
      */
    private def loadNodeFromStore(nodePtr: ArrB): F[Option[Node]] =
    ...
    /**
      ...
      */
    private val cacheR: TrieMap[ArrB, Node] = TrieMap.empty
    ...
    /**
      ...
      */
    def loadNode(nodePtr: ArrB, noAssert: Boolean = false): F[Node] =
      for {
    ...
    /**
      ...
      */
    private val cacheW: TrieMap[ArrB, ByteBuffer] = TrieMap.empty
...
//        case None => cacheR.update(hash, node)
    ...
    def commit(): F[Unit] = {
      ...
      for {
        ...
        kvCollision       = kvvExist.filter(kvv => !kvv._1._2.rewind().equals(kvv._2.rewind())).map(_._1)
