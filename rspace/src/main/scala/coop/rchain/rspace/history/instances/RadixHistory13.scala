...
/**
  ...
  */
object RadixHistory13 {
  def apply[F[_]: Sync: Parallel](
      rootHash: Blake2b256Hash,
      store: RadixStore5[F]
  ): F[RadixHistory13[F]] =
    for {
      impl     <- Sync[F].delay(new RadixTreeImpl13[F](store))
      rootNode <- impl.loadNode(rootHash.bytes, noAssert = true)
    } yield RadixHistory13(rootHash, rootNode, impl, store)
}

final case class RadixHistory13[F[_]: Sync: Parallel](
    rootHash: Blake2b256Hash,
    rootNode: Node,
    impl: RadixTreeImpl13[F],
    store: RadixStore5[F]
) extends History[F] {
