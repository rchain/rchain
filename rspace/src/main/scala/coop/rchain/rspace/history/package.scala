          ...
          tip match {
            ...
            case existingLeaf@Leaf(ek, _) if key != ek =>
              ...
              val hd = if(pathLeft > 1) {
                val skipNew = Skip(ByteVector(encodedKeyNew.splitAt(pathLength + 1)._2), LeafPointer(newLeafHash))
                val skipExisting = Skip(ByteVector(encodedKeyNew.splitAt(pathLength + 1)._2), LeafPointer(newLeafHash))
            ...
            case Node(pb) =>
              ...
              logger.debug(s"Node state, current:$pathLength, left: $pathLeft, whole: $wholePathLength")
              val ptr = if (pathLeft > 1) {
                val skip = Skip(ByteVector(encodedKeyNew.splitAt(pathLength + 1)._2), LeafPointer(newLeafHash))
