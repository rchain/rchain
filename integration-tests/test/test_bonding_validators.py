...
def test_bonding_validators(command_line_options: CommandLineOptions, random_generator: Random, docker_client: DockerClient) -> None:
    ...
    with conftest.testing_context(command_line_options, random_generator, docker_client, validator_bonds_dict=bonded_validator_map, wallets_dict=genesis_vault) as context, \
        ready_bootstrap_with_network(context=context, synchrony_constraint_threshold=0, epoch_length=epoch_length, quarantine_length=quarantine_length, shard_id=shard_id) as bootstrap_node, \
        bootstrap_connected_peer(context=context, bootstrap=bootstrap_node, name='bonded-validator', private_key=BONDED_VALIDATOR_KEY, epoch_length=epoch_length, quarantine_length=quarantine_length, shard_id=shard_id) as bonded_validator:
            ...
            # block number 1
            bonded_validator.deploy(contract_path, BOOTSTRAP_KEY, shard_id=shard_id)
            ...
            with bootstrap_connected_peer(context=context, bootstrap=bootstrap_node, name='joining-validator', private_key=JOINING_VALIDATOR_KEY, shard_id=shard_id) as joining_validator:
                ...
                with pytest.raises(RClientException):
                    joining_validator.deploy(contract_path, JOINING_VALIDATOR_KEY, shard_id=shard_id)
                ...
                _, vault_remain = get_vault_balance(context, joining_validator, JOINING_VALIDATOR_KEY.get_public_key().get_rev_address(), JOINING_VALIDATOR_KEY, 100000, 1, shard_id=shard_id)
