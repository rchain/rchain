...
def test_transfer_failed_with_invalid_key(command_line_options: CommandLineOptions, docker_client: DockerClient, random_generator: Random) -> None:
    with testing_context(command_line_options, random_generator, docker_client) as context:
        with docker_network_with_started_bootstrap(context=context) as bootstrap_node:
            charlie_init_vault_value = 5000

            alice_rev_address = get_rev_address_by_public_key(context, bootstrap_node, ALICE_KEY.public_key, ALICE_KEY.private_key)
            charlie_rev_address = get_rev_address_by_public_key(context, bootstrap_node, BOB_KEY.public_key, CHARLIE_KEY.private_key)

            create_genesis_vault(context, bootstrap_node, charlie_rev_address, charlie_init_vault_value, ALICE_KEY.private_key)
            alice_balance = get_vault_balance(context, bootstrap_node, alice_rev_address, ALICE_KEY.private_key)
            charlie_balance = get_vault_balance(context, bootstrap_node, charlie_rev_address, BOB_KEY.private_key)

            assert charlie_balance == 5000
            assert alice_balance == 0

            result = transfer_funds(context, bootstrap_node, charlie_rev_address, alice_rev_address, 100, ALICE_KEY.private_key)
