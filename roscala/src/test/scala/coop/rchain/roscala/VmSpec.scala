...
class VmSpec extends FlatSpec with Matchers {
  ...
  "OpApplyCmd" should "not change ctxt after running primitive" in {
    ...
    /**
      ...
      */
    val codevec = Seq(
      OpApplyCmd(unwind = false, next = false, nargs = 2, primNum = 232) // fx+
    )
  ...
  "OpApplyPrimTag" should "copy primitive result to location" in {
    ...
    val codevec = Seq(
      ...
      OpImmediateLitToArg(literal = `1`, arg = 0),
      OpImmediateLitToArg(literal = `2`, arg = 1),
      OpApplyPrimTag(unwind = false, next = false, nargs = 2, primNum = 232, lit = 0) // fx+
    )

    val code = Code(litvec = Seq(LocTrgt), codevec = codevec)
  ...
  "OpRtnReg" should "copy ctxt.rslt to given register" in {
    ...
    Vm.run(ctxt, globalEnv, Vm.State())
    rtnCtxt.trgt shouldBe Fixnum(100)
  }
  ...
  "OpRtnArg" should "copy ctxt.rslt to given argvec element" in {
    ...
    val rtnCtxt = Ctxt.outstanding(1)
    rtnCtxt.argvec = Tuple(Niv, Niv)

    val code = Code(litvec = Seq(Fixnum(100)), codevec = codevec)
    val ctxt = Ctxt(code, rtnCtxt, LocRslt)
    Vm.run(ctxt, globalEnv, Vm.State())

    rtnCtxt.arg(1) shouldBe Fixnum(100)
  }
  ...
  "OpRtnTag" should "copy ctxt.rslt to given location" in {
    ...
    val code = Code(litvec = Seq(Fixnum(100), ArgRegister(1)), codevec = codevec)
  ...
  "OpLookupToReg" should "lookup binding in continuation when compiler can't find the compile-time binding for method." in {
    ...
    /**
      ...
      * And then it will be found in somewhere.
      *
      */
    val codevec = Seq(
      OpAlloc(n = 1),
