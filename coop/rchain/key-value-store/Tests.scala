...
object Tests {
  def TestsRecursive(storeFilePath: String): String = {
    ...
    queryOutcome = QueryTools.queryResultsToArrayString(query,
                                                        query.unifyQuery(store),
                                                        store)
    oracle = Array("{x:B(C(a),D(E(b))),y:F(c)} -> [Y]")
    EvaluateTest(queryOutcome, oracle)
    ...
    queryOutcome = QueryTools.queryResultsToArrayString(query,
                                                        query.unifyQuery(store),
                                                        store)
    oracle = Array("{x:a,y:D(E(b)),2:c} -> [Y]")
    EvaluateTest(queryOutcome, oracle)
    /*
    // A(B(C(x), 1), F(2))
    query = new Key("A(B(C(x), 1), F(2))")
    queryOutcome = QueryTools.queryResultsToArrayString(
      query, query.unifyQuery(store), store)
    oracle = Array("{x:a,1:D(E(b)),2:c} -> [Y]")
    EvaluateTest(queryOutcome, oracle)
     */
    "Completed tests"
  }
