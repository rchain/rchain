/*               __          __                                         *\
**    __________/ /_  ____ _/_/___                                      **
**   / ___/ ___/ __ \/ __ `/ / __ \     RChain API                      **
**  / /  / /__/ / / / /_/ / / / / /     (c) http://rchain.coop          **
** /_/   \___/_/ /_/\____/_/_/ /_/                                      **
\*                                                                      */

package Storage

import java.nio.ByteBuffer
import java.nio.ByteBuffer.allocateDirect
import java.nio.charset.StandardCharsets.UTF_8

// Java ByteBuffer
object Bb {

  def toBb[T](value: T): Option[ByteBuffer] = {
    if (value.isInstanceOf[Byte]) {
      ???
    } else if (value.isInstanceOf[Boolean]) {
      ???
    } else if (value.isInstanceOf[Char]) {
      ???
    } else if (value.isInstanceOf[Short]) {
      return Some(shortToBb(value.asInstanceOf[Short]))
    } else if (value.isInstanceOf[Int]) {
      return Some(intToBb(value.asInstanceOf[Int]))
    } else if (value.isInstanceOf[Long]) {
      return Some(longToBb(value.asInstanceOf[Long]))
    } else if (value.isInstanceOf[Float]) {
      return Some(floatToBb(value.asInstanceOf[Float]))
    } else if (value.isInstanceOf[Double]) {
      return Some(doubleToBb(value.asInstanceOf[Double]))
    } else if (value.isInstanceOf[String]) {
      return Some(strToBb(value.asInstanceOf[String]))
    }
    None
  }

  def shortToBb(value: Short): ByteBuffer = {
    val bb = ByteBuffer.allocateDirect(java.lang.Short.BYTES)
    bb.putInt(value).flip
    bb
  }

  def intToBb(value: Int): ByteBuffer = {
    // Notice assumed equivalence of Int and Integer
    val bb = ByteBuffer.allocateDirect(java.lang.Integer.BYTES)
    bb.putInt(value).flip
    bb
  }

  def longToBb(value: Long): ByteBuffer = {
    val bb = ByteBuffer.allocateDirect(java.lang.Long.BYTES)
    bb.putLong(value).flip
    bb
  }

  def floatToBb(value: Float): ByteBuffer = {
    val bb = ByteBuffer.allocateDirect(java.lang.Float.BYTES)
    bb.putFloat(value).flip
    bb
  }

  def doubleToBb(value: Double): ByteBuffer = {
    val bb = ByteBuffer.allocateDirect(java.lang.Double.BYTES)
    bb.putDouble(value).flip
    bb
  }

  def strToBb(str: String): ByteBuffer = {
    var bb = allocateDirect(str.length)
    bb.put(str.getBytes(UTF_8)).flip
    bb
  }

  def bbToStr(buffer: ByteBuffer): String = {
    /*
    https://worldmodscode.wordpress.com/2012/12/14/the-java-bytebuffer-a-crash-course/
    suggests that the use of position() introduces a race condition a bug.
    Search for the third instance of "// NOT RECOMMENDED, don't do this"
     */
    val oldPosition = buffer.position
    val str = UTF_8.decode(buffer).toString()
    // reset buffer's position to its original so it is not altered:
    buffer.position(oldPosition)
    str
  }

  def equalsString(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = {
    val a = bbToStr(aIn)
    val b = bbToStr(bIn)
    a == b
  }

  def equalsInt(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = {
    val a = aIn.getInt()
    val b = bIn.getInt()
    a == b
  }

  def equalsByte(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = ???

  def equalsBoolean(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = ???

  def equalsChar(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = {
    val a = aIn.getChar()
    val b = bIn.getChar()
    a == b
  }

  def equalsShort(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = {
    val a = aIn.getShort()
    val b = bIn.getShort()
    a == b
  }

  def equalsLong(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = {
    val a = aIn.getLong()
    val b = bIn.getLong()
    a == b
  }

  def equalsFloat(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = {
    val a = aIn.getFloat()
    val b = bIn.getFloat()
    a == b
  }

  def equalsDouble(aIn: ByteBuffer, bIn: ByteBuffer): Boolean = {
    val a = aIn.getDouble()
    val b = bIn.getDouble()
    a == b
  }
}

object BbTests {

  def tests(): Unit = {

    println("BbTests.tests() begin")

    types()
    equals()

    println("BbTests.tests() end")
  }

  def types(): Unit = {

    println("types() begin")

    // val bbShort = Bb.toBb(3.toShort)
    // assert(bbShort.get.getShort() == 3.toShort)

    val bbInt = Bb.toBb(7)
    assert(bbInt.get.getInt() == 7)

    val bbLong = Bb.toBb(11.toLong)
    assert(bbLong.get.getLong() == 11)

    val bbFloat = Bb.toBb(1.2.toFloat)
    assert(bbFloat.get.getFloat() == 1.2.toFloat)

    val bbDouble = Bb.toBb(3.5.toDouble)
    assert(bbDouble.get.getDouble() == 3.5.toDouble)

    val bbStr = Bb.strToBb("test")
    assert(Bb.bbToStr(bbStr) == "test")

    println("types() end")
  }

  def equals(): Unit = {

    println("equals() begin")

    assert(Bb.equalsInt(Bb.toBb(7).get, Bb.toBb(7).get))
    assert(Bb.equalsLong(Bb.toBb(11.toLong).get, Bb.toBb(11.toLong).get))
    assert(Bb.equalsFloat(Bb.toBb(1.2.toFloat).get, Bb.toBb(1.2.toFloat).get))
    assert(
      Bb.equalsDouble(Bb.toBb(3.5.toDouble).get, Bb.toBb(3.5.toDouble).get))
    assert(Bb.equalsString(Bb.strToBb("test"), Bb.strToBb("test")))

    println("equals() end")
  }
}
