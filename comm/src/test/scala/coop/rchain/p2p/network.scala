...
class ProtocolTest extends FlatSpec with Matchers {
  "A Handshake message" should "be answered by a HandshakeResponse message" in {
    ...
    val hs = HandshakeMessage(NetworkProtocol.handshake(src), System.currentTimeMillis)
    ...
    val hsr = upstream.unpack(HandshakeResponse)
