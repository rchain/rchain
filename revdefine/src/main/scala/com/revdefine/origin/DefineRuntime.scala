...
import com.revdefine.node.setup.Setup.RevDefineServices
...
import coop.rchain.node.runtime.{APIServers, NodeCallCtx, ServersInstances}
...
class DefineRuntime[F[_]: Monixable: ConcurrentEffect: Parallel: Timer: ContextShift: LocalEnvironment: Log: EventLog](
    nodeConf: NodeConf,
    kamonConf: Config,
    id: NodeIdentifier,
    scheduler: Scheduler
  ...
  private def nodeProgram(
      ...
      revDefineServices: RevDefineServices[F]
  )(
      implicit
      time: Time[F],
    ...
    for {
      ...
      node = fs2
        .Stream(
          ...
          revDefineServices.toRunningStream
        )
        .parJoinUnbounded
...
object DefineRuntime {
  ...
  def start[F[_]: Monixable: ConcurrentEffect: Parallel: ContextShift: Timer: Log: EventLog](
    ...
    for {
      ...
      runtime = new DefineRuntime[TaskEnv](nodeConf, kamonConf, id, scheduler)
