...
class ValidateTest
  ...
  "Block signature validation" should "return false on unknown algorithms" in withStore[Task, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val chain                    = createChain[StateWithChain](2).runS(initState).runSyncUnsafe(1.second)
      ...
      Validate.blockSignature[Task](block0).runSyncUnsafe(1.second) should be(false)
      log.warns.last.contains(s"signature algorithm $unknownAlgorithm is unsupported") should be(
        true
      )

      Validate.blockSignature[Task](block1).runSyncUnsafe(1.second) should be(false)
      log.warns.last.contains(s"signature algorithm $rsa is unsupported") should be(true)
  }

  it should "return false on invalid ed25519 signatures" in withStore[Task, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      implicit val chain           = createChain[StateWithChain](6).runS(initState).runSyncUnsafe(1.second)
  ...
  it should "return true on valid ed25519 signatures" in withStore[Task, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val n                        = 6
      implicit val chain           = createChain[StateWithChain](n).runS(initState).runSyncUnsafe(1.second)
      implicit val (sk, _)         = Ed25519.newKeyPair
  ...
  "Timestamp validation" should "not accept blocks with future time" in withStore[Task, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val chain                    = createChain[StateWithChain](1).runS(initState).runSyncUnsafe(1.second)
      ...
      Validate.timestamp[Task](block, chain).runSyncUnsafe(1.second) should be(Right(Valid))

      log.warns.size should be(1)
      log.warns.head.contains("block timestamp") should be(true)
  }

  it should "not accept blocks that were published before parent time" in withStore[Task, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val chain                    = createChain[StateWithChain](2).runS(initState).runSyncUnsafe(1.second)
  ...
  "Block number validation" should "only accept 0 as the number for a block with no parents" in withStore[
    ...
    Validate.blockNumber[Task](block.withBlockNumber(1)).runSyncUnsafe(1.second) should be(
      Left(InvalidBlockNumber)
    )
    Validate.blockNumber[Task](block).runSyncUnsafe(1.second) should be(Right(Valid))
    log.warns.size should be(1)
    log.warns.head.contains("not zero, but block has no parents") should be(true)
  }

  it should "return false for non-sequential numbering" in withStore[Task, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val chain                    = createChain[StateWithChain](2).runS(initState).runSyncUnsafe(1.second)
      val block                    = chain.idToBlocks(1)

      Validate.blockNumber[Task](block.withBlockNumber(17)).runSyncUnsafe(1.second) should be(
        Left(InvalidBlockNumber)
      )
      Validate.blockNumber[Task](block).runSyncUnsafe(1.second) should be(Right(Valid))
      log.warns.size should be(1)
  ...
  it should "return true for sequential numbering" in withStore[Task, Unit] { implicit blockStore =>
    implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
    val n                        = 6
    val chain                    = createChain[StateWithChain](n).runS(initState).runSyncUnsafe(1.second)

    (0 until n).forall(
      i => Validate.blockNumber[Task](chain.idToBlocks(i)).runSyncUnsafe(1.second) == Right(Valid)
    ) should be(
      true
    )
    log.warns should be(Nil)
  ...
  it should "correctly validate a multiparent block where the parents have different block numbers" in withStore[
    ...
    Validate.blockNumber[Task](b3).runSyncUnsafe(1.second) shouldBe Right(Valid)
    Validate.blockNumber[Task](b3.withBlockNumber(4)).runSyncUnsafe(1.second) shouldBe Left(
      InvalidBlockNumber
    )
  }
  ...
  "Sequence number validation" should "only accept 0 as the number for a block with no parents" in withStore[
    ...
    Validate.sequenceNumber[Task](block.withSeqNum(1), chain).runSyncUnsafe(1.second) should be(
      Left(InvalidSequenceNumber)
    )
    Validate.sequenceNumber[Task](block, chain).runSyncUnsafe(1.second) should be(Right(Valid))
    log.warns.size should be(1)
  }
  ...
  it should "return true for sequential numbering" in withStore[Task, Unit] { implicit blockStore =>
    ...
    (0 until n).forall(
      i =>
        Validate.sequenceNumber[Task](chain.idToBlocks(i), chain).runSyncUnsafe(1.second) == Right(
          Valid
        )
    ) should be(
      true
  ...
  "Sender validation" should "return true for genesis and blocks from bonded validators and false otherwise" in withStore[
    ...
    Validate.blockSender[Task](genesis, genesis, chain).runSyncUnsafe(1.second) should be(true)
    Validate.blockSender[Task](validBlock, genesis, chain).runSyncUnsafe(1.second) should be(true)
    Validate.blockSender[Task](invalidBlock, genesis, chain).runSyncUnsafe(1.second) should be(
      false
    )
  }
  ...
  "Parent validation" should "return true for proper justifications and false otherwise" in withStore[
    ...
    val chain = createChainWithValidators[StateWithChain].runS(initState).runSyncUnsafe(1.second)
    val b0    = chain.idToBlocks(0)

    (0 to 6).forall(
      i =>
        Validate.parents[Task](chain.idToBlocks(i), b0, chain).runSyncUnsafe(1.second) == Right(
          Valid
        )
    ) should be(
      true
  ...
  "Block summary validation" should "short circuit after first invalidity" in withStore[Task, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val chain                    = createChain[StateWithChain](2).runS(initState).runSyncUnsafe(1.second)
      val block                    = chain.idToBlocks(1)
      ...
      Validate
        ...
        .runSyncUnsafe(1.second) should be(Left(InvalidBlockNumber))
      log.warns.size should be(1)
  }
    ...
    (1 to 6).forall(
      i =>
        Validate
          ...
          .runSyncUnsafe(1.second) == Right(Valid)
    ) should be(true)

    Validate
      .justificationFollows[Task](chain.idToBlocks(7), genesis, chain)
      .runSyncUnsafe(1.second) == Left(InvalidFollows) should be(
      true
    )
  ...
  "Bonds cache validation" should "succeed on a valid block and fail on modified bonds" in withStore[
    ...
    Validate.bondsCache[Task](genesis, runtimeManager).runSyncUnsafe(1.second) should be(
      Right(Valid)
    )
    ...
    Validate.bondsCache[Task](modifiedGenesis, runtimeManager).runSyncUnsafe(1.second) should be(
  ...
  "Field format validation" should "succeed on a valid block and fail on empty fields" in {
    ...
    Validate.formatOfFields[Task](genesis).runSyncUnsafe(1.second) should be(true)
    Validate
      .formatOfFields[Task](genesis.withBlockHash(ByteString.EMPTY))
      .runSyncUnsafe(1.second) should be(false)
    Validate.formatOfFields[Task](genesis.clearHeader).runSyncUnsafe(1.second) should be(false)
    Validate.formatOfFields[Task](genesis.clearBody).runSyncUnsafe(1.second) should be(false)
    Validate
      .formatOfFields[Task](genesis.withSig(ByteString.EMPTY))
      .runSyncUnsafe(1.second) should be(false)
    Validate.formatOfFields[Task](genesis.withSigAlgorithm("")).runSyncUnsafe(1.second) should be(
      false
    )
    Validate.formatOfFields[Task](genesis.withShardId("")).runSyncUnsafe(1.second) should be(false)
    Validate
  ...
  "Block hash format validation" should "fail on invalid hash" in {
    ...
    Validate.blockHash[Task](genesis).runSyncUnsafe(1.second) should be(Right(Valid))
    Validate
      .blockHash[Task](
        genesis.withBlockHash(ByteString.copyFromUtf8("123"))
      )
      .runSyncUnsafe(1.second) should be(Left(InvalidBlockHash))
  }
  ...
  "Block deploy count validation" should "fail on invalid number of deploys" in {
    ...
    val block    = HashSetCasperTest.createGenesis(Map(pk -> 1))
    val genesis  = ProtoUtil.signBlock(block, BlockDag.empty, pk, sk, "ed25519", "rchain")
    Validate.deployCount[Task](genesis).runSyncUnsafe(1.second) should be(Right(Valid))
    Validate
      .deployCount[Task](
        genesis.withHeader(genesis.header.get.withDeployCount(100))
      )
      .runSyncUnsafe(1.second) should be(Left(InvalidDeployCount))
  }
  ...
  "Block version validation" should "work" in {
    ...
    val block    = HashSetCasperTest.createGenesis(Map(pk -> 1))
    val genesis  = ProtoUtil.signBlock(block, BlockDag.empty, pk, sk, "ed25519", "rchain")
    Validate.version[Task](genesis, -1).runSyncUnsafe(1.second) should be(false)
    Validate.version[Task](genesis, 1).runSyncUnsafe(1.second) should be(true)
  }
}
