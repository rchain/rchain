...
class CliqueOracleTest extends FlatSpec with Matchers with BlockGenerator with BlockStoreFixture {
  ...
  "Turan Oracle" should "detect finality as appropriate" in withStore[Task, Unit] {
    implicit blockStore =>
      ...
      val chain: IndexedBlockDag =
        createChain[StateWithChain].runS(initState).runSyncUnsafe(1.second)
      ...
      runSafetyOracle[Task]
  }

  // See [[/docs/casper/images/no_finalizable_block_mistake_with_no_disagreement_check.png]]
  "Turan Oracle" should "detect possible disagreements appropriately" in withStore[Task, Unit] {
    implicit blockStore =>
      implicit val blockStoreChain = storeForStateWithChain[StateWithChain](blockStore)
      val v1                       = ByteString.copyFromUtf8("Validator One")
      val v2                       = ByteString.copyFromUtf8("Validator Two")
