...
class RepeatedReplayRSpaceTest extends FlatSpec with Matchers {
  ...
  "ReplayRSpace" should "not have untraced comm events" in {
    ...
    val chs = repeatedChain(n, code, rm1, rm2)
...
object RepeatedReplayRSpaceTest {
  implicit val scheduler = Scheduler.singleThread("rspace-test")

  def mkManager(): RuntimeManager = {
    val active = TestSetUtil.runtime("test", size = 1024L * 1024 / 2)
  ...
  private def useHash(hash: ByteString)(
      code: String,
      rm1: RuntimeManager,
      rm2: RuntimeManager): Either[Checkpoint, (ByteString, Checkpoint)] = {
    val (_, ch) = eval(code, hash, rm1)
    //val chRep   = replay(d, hash, ch.log, rm2)
    if (ch.log.size != 54) Left(ch)
    else
      Right(ByteString.copyFrom(ch.root.bytes.toArray) -> ch)
  }

  @tailrec
  def repeatedChain(n: Int,
                    code: String,
                    rm1: RuntimeManager,
                    rm2: RuntimeManager,
    ...
    if (n == 0) acc
      ...
      start match {
        case None =>
          useHash(rm1.emptyStateHash)(code, rm1, rm2) match {
            case Right((newStart, ch)) =>
              repeatedChain(n - 1, code, rm1, rm2, Some(newStart), ch :: acc)
            case Left(ch) => ch :: acc
          }

        case Some(hash) =>
          useHash(hash)(code, rm1, rm2) match {
            case Right((newStart, ch)) =>
              repeatedChain(n - 1, code, rm1, rm2, Some(newStart), ch :: acc)
            case Left(ch) => ch :: acc
          }
  ...
  def repeatFromEmpty(n: Int,
    ...
    if (n == 0) acc
      ...
      useHash(rm1.emptyStateHash)(code, rm1, rm2) match {
        case Right((_, ch)) => repeatFromEmpty(n - 1, code, rm1, rm2, ch :: acc)
        case Left(ch)       => ch :: acc
      }
