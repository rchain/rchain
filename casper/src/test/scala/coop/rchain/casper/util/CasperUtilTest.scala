...
class CasperUtilTest extends FlatSpec with Matchers with BlockGenerator {
  ...
  "isInMainChain" should "classify complicated chains appropriately" in {
    ...
    val createChain =
      for {
        genesis <- createBlock[StateWithChain](Seq(), ByteString.EMPTY)
        b2      <- createBlock[StateWithChain](Seq(genesis.blockHash), v2)
        _       <- createBlock[StateWithChain](Seq(genesis.blockHash), v1)
        b4      <- createBlock[StateWithChain](Seq(b2.blockHash), v2)
        _       <- createBlock[StateWithChain](Seq(b2.blockHash), v1)
        _       <- createBlock[StateWithChain](Seq(b4.blockHash), v2)
        b7      <- createBlock[StateWithChain](Seq(b4.blockHash), v1)
        b8      <- createBlock[StateWithChain](Seq(b7.blockHash), v1)
      } yield b8
