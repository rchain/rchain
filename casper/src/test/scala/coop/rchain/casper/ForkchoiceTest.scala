...
class ForkchoiceTest extends FlatSpec with Matchers with BlockGenerator with BlockStoreFixture {
  ...
  implicit val metricsTask: Metrics[Task] = new MetricsNOP[Task]
  ...
  "Estimator on empty latestMessages" should "return the genesis regardless of DAG" in withStore[
    ...
    val chain: IndexedBlockDag = createChain[StateWithChain].runS(initState).runSyncUnsafe(1.second)
    ...
    checkForkchoice[Task].runSyncUnsafe(1.second)
  }
