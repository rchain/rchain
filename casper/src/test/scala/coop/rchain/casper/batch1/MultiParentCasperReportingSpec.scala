    ...
    TestNode.standaloneEff(genesis).use { node =>
      ...
      for {
        ...
        reportingCommEventsNum = trace match {
          case Right(value) =>
            value.deployReportResult.head.processedDeploy.deployLog.foldLeft(0)(
              (sum, e) =>
                e match {
                  case CommEvent(_, _, _) => sum + 1
                  case _                  => sum
                }
            )
          case Left(_) => 0
        ...
        reportingReplayPostStateHash = trace.toOption.map(_.postStateHash).get
