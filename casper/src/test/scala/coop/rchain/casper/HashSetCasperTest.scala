...
class HashSetCasperTest extends FlatSpec with Matchers {
  ...
  it should "not allow multiple threads to process the same block" in {
    ...
    val testProgram = for {
      ...
      result <- EitherT(
                 Task
                   .racePair(
                     casper.addBlock(block, kp2(().pure[Effect])).value,
                     casper.addBlock(block, kp2(().pure[Effect])).value
                   )
                   .flatMap {
  ...
  it should "be able to use the registry" in effectTest {
    ...
    for {
      ...
      Created(block)    = createBlockResult
      blockStatus       <- casperEff.addBlock(block, kp2(().pure[Effect]))
      id <- casperEff
      ...
      block2Status       <- casperEff.addBlock(block2, kp2(().pure[Effect]))
  ...
  it should "be able to create a chain of blocks from different deploys" in effectTest {
    ...
    for {
      ...
      _                     <- MultiParentCasper[Effect].addBlock(signedBlock1, kp2(().pure[Effect]))
      ...
      _                     <- MultiParentCasper[Effect].addBlock(signedBlock2, kp2(().pure[Effect]))
  ...
  it should "allow multiple deploys in a single block" in effectTest {
    ...
    for {
      ...
      createBlockResult <- MultiParentCasper[Effect].createBlock
      Created(block)    = createBlockResult
      _                 <- MultiParentCasper[Effect].addBlock(block, kp2(().pure[Effect]))
  ...
  it should "reject unsigned blocks" in effectTest {
    ...
    for {
      ...
      _              <- MultiParentCasper[Effect].addBlock(invalidBlock, kp2(().pure[Effect]))
  ...
  it should "propose blocks it adds to peers" in effectTest {
    for {
      ...
      Created(signedBlock) = createBlockResult
      _                    <- nodes(0).casperEff.addBlock(signedBlock, kp2(().pure[Effect]))
      _                    <- nodes(1).receive()
  ...
  it should "add a valid block from peer" in effectTest {
    for {
      ...
      _                          <- nodes(0).casperEff.addBlock(signedBlock1Prime, kp2(().pure[Effect]))
      _                          <- nodes(1).receive()
  ...
  it should "handle multi-parent blocks correctly" in effectTest {
    for {
      ...
      _                  <- nodes(0).casperEff.addBlock(block0, kp2(().pure[Effect]))
      _                  <- nodes(1).casperEff.addBlock(block1, kp2(().pure[Effect]))
      _                  <- nodes(0).receive()
      _                  <- nodes(1).receive()
      _                  <- nodes(0).receive()
      _                  <- nodes(1).receive()
      ...
      _                         <- nodes(0).casperEff.addBlock(multiparentBlock, kp2(().pure[Effect]))
      _                         <- nodes(1).receive()
  ...
  it should "not merge blocks that touch the same channel" in effectTest {
    for {
      ...
      _                          <- nodes(0).casperEff.addBlock(singleParentBlock, kp2(().pure[Effect]))
      _                          <- nodes(1).receive()
  ...
  it should "allow bonding and distribute the joining fee" in effectTest {
    for {
      ...
      Created(block1) = createBlock1Result
      block1Status    <- nodes(0).casperEff.addBlock(block1, kp2(().pure[Effect]))
      ...
      block2Status    <- nodes(1).casperEff.addBlock(block2, kp2(().pure[Effect]))
      ...
      block3Status    <- nodes.last.casperEff.addBlock(block3, kp2(().pure[Effect]))
      ...
      block3PrimeStatus    <- nodes.head.casperEff.addBlock(block3Prime, kp2(().pure[Effect]))
  ...
  it should "allow bonding via the faucet" in effectTest {
    ...
    for {
      ...
      Created(block1)    = createBlockResult1
      block1Status       <- casperEff.addBlock(block1, kp2(().pure[Effect]))
  ...
  it should "not fail if the forkchoice changes after a bonding event" in effectTest {
    ...
    for {
      ...
      bondedBlockStatus <- nodes.head.casperEff.addBlock(bondedBlock, kp2(().pure[Effect]))
      _                 <- nodes(1).receive()
      ...
      Created(block2) = createBlockResult2
      status2         <- nodes(1).casperEff.addBlock(block2, kp2(().pure[Effect]))
      _               <- nodes.head.receive()
      _               <- nodes(1).receive()
      ...
      status3         <- nodes(2).casperEff.addBlock(block3, kp2(().pure[Effect]))
      _               <- nodes.toList.traverse_(_.receive())
      ...
      status4         <- nodes.head.casperEff.addBlock(block4, kp2(().pure[Effect]))
      _               <- nodes.toList.traverse_(_.receive())
  ...
  it should "reject addBlock when there exist deploy by the same (user, millisecond timestamp) in the chain" in effectTest {
    for {
      ...
      _                     <- nodes(0).casperEff.addBlock(signedBlock3, kp2(().pure[Effect]))
      ...
      _                     <- nodes(1).casperEff.addBlock(signedBlock4, kp2(().pure[Effect])) // should succeed
  ...
  it should "not ignore equivocation blocks that are required for parents of proper nodes" in effectTest {
    for {
      ...
      _ <- nodes(1).casperEff.addBlock(signedBlock1, kp2(().pure[Effect]))
      ...
      _ <- nodes(2).casperEff.addBlock(signedBlock3, kp2(().pure[Effect]))
      _ <- nodes(1).casperEff.addBlock(signedBlock2, kp2(().pure[Effect]))
      ...
      Created(signedBlock4) = createBlockResult4
      _                     <- nodes(1).casperEff.addBlock(signedBlock4, kp2(().pure[Effect]))
  ...
  it should "prepare to slash an block that includes a invalid block pointer" in effectTest {
    for {
      ...
      _ <- nodes(1).casperEff.addBlock(blockWithInvalidJustification, kp2(().pure[Effect]))
    ...
    for {
      ...
      _ <- (0 to 9).toList.traverse_[Effect, Unit] { i =>
            for {
              ...
              _ <- nodes(0).casperEff.addBlock(block, kp2(().pure[Effect]))
              _ <- nodes(1).transportLayerEff.clear(nodes(1).local) //nodes(1) misses this block
            } yield ()
          }
      ...
      _                <- nodes(0).casperEff.addBlock(block11, kp2(().pure[Effect]))
  ...
  it should "increment last finalized block as appropriate in round robin" in effectTest {
    ...
    for {
      ...
      _               = nodes(1).casperEff.addBlock(block8, kp2(().pure[Effect]))
      _               = nodes(0).receive()
      _               = nodes(2).receive()
...
object HashSetCasperTest {
  ...
  def deployAndQuery(
    ...
    for {
      ...
      Created(block)    = createBlockResult
      blockStatus       <- node.casperEff.addBlock(block, kp2(().pure[Effect]))
