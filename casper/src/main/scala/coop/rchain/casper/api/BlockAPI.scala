...
object BlockAPI {
  ...
  private def getBlockInfo[A, F[_]: Monad: MultiParentCasper: SafetyOracle: BlockStore](
    ...
    for {
      ...
      bondsValidatorList       = ProtoUtil.bonds(block).map(bond => bond.validator)
  ...
  private def constructBlockInfo[F[_]: Monad: MultiParentCasper: SafetyOracle: BlockStore](
      ...
      initialFault: Float,
      bondsValidatorList: Seq[Validator]
  ): F[BlockInfo] =
    for {
      ...
      BlockInfo(
        ...
        bondsValidatorList = bondsValidatorList.map(PrettyPrinter.buildStringNoLimit)
      )
