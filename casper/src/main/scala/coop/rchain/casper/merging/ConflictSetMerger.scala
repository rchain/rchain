...
object ConflictSetMerger {
  ...
  def merge[F[_]: Concurrent, R](
      ...
      applyTrieActionsToBase: Seq[HotStoreTrieAction] => F[Blake2b256Hash]
    ...
    for {
      ...
      prodAndConsTrieActions <- computeTrieActions(baseState, allChanges)
      newState               <- applyTrieActionsToBase(prodAndConsTrieActions)
