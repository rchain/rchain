...
object DeployChainMerger {
  ...
  /**
    ...
    */
  def merge[F[_]: Concurrent](baseState: Blake2b256Hash, mergeSet: Set[DeployChain])(
      runtimeManager: RuntimeManager[F]
  ): F[(Blake2b256Hash, Int, String, String)] =
    for {
      indices     <- mergeSet.toList.traverse(getDeployChainIndex[F])
      baseReader  <- runtimeManager.getHistoryRepo.getHistoryReader(baseState).map(_.readerBinary)
