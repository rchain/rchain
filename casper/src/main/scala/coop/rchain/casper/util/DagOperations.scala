...
object DagOperations {
  ...
  //gca(b1, b2) satifies the following properties:
  //1. it is an ancestor of both b1 and b2
  //2. there does not exist a child of it which is an ancestor of both b1 and b2
  //3. any path from b1 to genesis (i.e. any parent can be chosen when traversing the DAG) includes it
  //4. likewise for b2
  //Conceptually, it is the first point at which the histories of b1 and b2 diverge.
  ...
  def greatestCommonAncestor(b1: BlockMessage,
    ...
    gca.getOrElse(b1) //none found iff b1 == b2
  }
}
