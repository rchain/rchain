...
import coop.rchain.v2.casper.Casper.{ConflictScope, Finalize, MessageScope, NoFinalizationFringe}
...
object DeployChainSetCasper1 {
  ...
  def computeMessageScope[F[_]: Sync](
    ...
    Sync[F].handleErrorWith(casper.casperScope(latestMessages, metaDag, safetyOracle)) {
      case NoFinalizationFringe =>
        for {
          ...
          gsFailed       =
            s"Genesis is out of reach of the highest message (Casper depth is $casperMaxDepth) " +
