...
Η Rholang είναι "προσανατολισμένη στη διαδικασία": όλοι οι υπολογισμοί γίνονται με μηνύματα. Τα μηνύματα μεταφέρονται σε "κανάλια", τα οποία μοιάζουν με ουρές μηνυμάτων αλλά συμπεριφέρονται ως σύνολα αντί για ουρές. Η Rholang είναι εντελώς ασύγχρονη υπό την έννοια ότι ενώ μπορείτε να διαβάσετε ένα μήνυμα από ένα κανάλι και στη συνέχεια να κάνετε κάτι με αυτό, δεν μπορείτε να στείλετε ένα μήνυμα και στη συνέχεια να κάνετε κάτι όταν θα έχει ληφθεί---τουλάχιστον, όχι χωρίς ρητή αναμονή μηνύματος επιβεβαίωσης από τον δέκτη.

## Συμβόλαια, προβληματισμός και μετάδοση δεδομένων

    1 contract @"HelloWorld"(system) = {
    2     system!("print", "Hello, world!")
    3 }

1) Στο Διαδίκτυο, οι διακομιστές έχουν διευθύνσεις IP. Η υπηρεσία ονομάτων τομέα (DNS) χαρτογραφεί αλφαριθμητικές συμβολοσειρές σε αριθμούς, όπως και σε έναν τηλεφωνικό κατάλογο. Αντί να χρησιμοποιεί αριθμούς ή συμβολοσειρές, η Rholang είναι "ανακλαστική": όλα τα κανάλια παίρνουν το όνομα τους από μια σειριακή διαδικασία. Όλες οι σειριοποιήσεις διεργασιών αρχίζουν με ένα `@`. Αυτό το συμβόλαιο ακούει για μηνύματα που αποστέλλονται στο κανάλι που έχουν ονομαστεί από τη σειριακή διαδικασία `"HelloWorld"`. Για να είμαστε ακριβείς, λέμε, "Αυτή η σύμβαση ακούει στο όνομα `@"HelloWorld"`."

Τα συμβόλαια διακυρήσουν ένα API με το οποίο άλλες διαδικασίες μπορούν να αλληλεπιδράσουν μαζί του. Τα συμβόλαια υψηλού επιπέδου έχουν όλα το ίδιο API: έχουν ένα επιχείρημα, το κανάλι για τη διαδικασία `system`. Η διαδικασία `system` περιέχει τα ονόματα όλων των καναλιών που μπορούν να προκαλέσουν παρενέργειες στον κόμβο. Αυτή είναι μια ολοκληρωμένη διαδικασία που ακούει για μηνύματα που αποτελούνται από ένα όνομα μεθόδου και κάποια επιχειρήματα.

2) Ο χειριστής θαυμαστικών στέλνει το μήνυμα στα δεξιά του πάνω απο το καναλι στα αριστερά του καναλιού. Κάθε μήνυμα είναι μια πλειάδα ονομάτων. Αν στείλουμε μια διαδικασία αντί για ένα όνομα, γίνεται αυτόματα σειριοποίηση σε ένα όνομα.

Σε αυτή την περίπτωση, στέλνουμε ένα μήνυμα που αποτελείται από δύο διαδικασίες, τη συμβολοσειρά `"print"` και τη συμβολοσειρά `"Hello, world!"` Η διαδικασία `system` είναι μια ενσωματωμένη διαδικασία που ακούει για μηνύματα που αποτελούνται από ένα όνομα μεθόδου και κάποια επιχειρήματα· στην περίπτωση αυτή στέλνει το το δεύτερο επιχείρημα στην standard έξοδο.
...
1) Για να δημιουργήσουμε ένα νέο, ιδιωτικό κανάλι, χρησιμοποιούμε την construction `new ... in`. Καμία άλλη διαδικασία δεν μπορεί να στείλει ή να λάβει μηνύματα μέσω αυτού του καναλιού, εκτός αν κάνουμε ρητή αποστολή αυτού του κανάλιου στην άλλη διαδικασία.
...
## Μεταβλητή κατάσταση, αναπαραγωγή και επιλογή
     ...
     2     // Δημιούργησε ενα κελί στο οποίο μπορείς να αποθυκεύεις τιμές
...
5) Παράλληλα με την γραμμη 4, προσπαθούμε να διαβάσουμε από το `valueStore`. Το διπλό βέλος μας λέει ότι μόλις λάβουμε ένα μήνυμα σε αυτό το κανάλι, θα πρέπει να δημιουργήσουμε ένα αντίγραφο της διαδικασίας αμέσως μετά το `for` και να προσπαθήσουμε το να ξανά διαβάσουμε αυτό το κανάλι αμέσως.
...
Αν υπάρχει μήνυμα στο κανάλι `get ', τότε το κλαδί στη γραμμή 7 μπορεί να τρέξει. Η μεταβλητή `ack` συνδέεται με το μήνυμα και  παράλληλα συμβαίνουν δύο πράγματα: η τιμή που διαβάζουμε ξανα στέλνετε στη `valueStore` και στο κανάλι `ack`.
...
2) Οι αγκύλες υποδυκνείουν μια λίστα. Οι λίστες είναι μεταβλητές, ενώ οι πλειάδες, που υποδικνείωνται με παρενθέσεις, δεν είναι. Ορισμένες διαδικασίες, όπως αυτές που υποστηρίζονται από αντικείμενα της Java, έχουν μεθόδους· εδώ η μέθοδος `iterate` γίνεται επίκληση με ένα κανάλι για εναλλαγή.
...
Η μέθοδος `print` της διαδικασίας του συστήματος μπορεί να πάρει ένα ή δύο επιχειρήματα. Στην περίπτωση δύο παραμέτρων, το δεύτερο όρισμα είναι ένα κανάλι στο οποίο θα αποσταλεί ένα μήνυμα επιβεβαίωσης μόλις ολοκληρωθεί η εκτύπωση. Η μέθοδος `iterate` θα λάβει αυτή την επιβεβαίωση και θα στείλει το επόμενο μήνυμα. Μόλις τελειώσει η λίστα, η γραμμή 3 θα εξελιχθεί στη διαδικασία `Nil` που δεν κάνει τίποτα και συλλέγει σκουπίδια.

## Pattern matching και παράμετροι rest
    ...
    36         for (cc <- ret) {
    37             // Δημιουργεί ενα νέο κελί με αρχική τιμή 0
    38             cc!("new", *ret, 0) |
    39             for (ticket <- ret) {
...
2) Ένα μοτίβο σχεδιασμού, που χρησιμοποιείται παραπάνω στο συμβόλαιο MakeCell, είναι να λαμβάνει από τον καλούντα ένα κανάλι για κάθε διαφορετική λειτουργικότητα που μια διαδικασία παρέχει. Ένας προγραμματιστής αντικειμενοστρεφούς γλώσσας μπορεί να πει ότι η MakeCell απαιτεί από τον καλούντα να παρέχει ένα κανάλι για κάθε μέθοδο. Οι αντιστοιχίσεις επιχειρούνται με τη σειρά που εμφανίζονται στον κώδικα· αν δεν συμβαίνει καμία αντιστοίχιση, το μπλοκ `match` ορίζεται στη διαδικασία `Nil`.

3) Το MakeCoatCheck χρησιμοποιεί μια πιο αντικειμενοστρεφή προσέγγιση. Η κατασκευή `(in, out):iopair` μας επιτρέπει να δημιουργήσουμε ένα συζευγμένο ζευγάρι καναλιών. Πρόκειται για σφάλμα τύπου για την αποστολή μηνύματος μέσω του καναλιού `in` ή για τη λήψη μηνύματος μέσω του καναλιού` out`. Ωστόσο, κάθε μήνυμα που αποστέλλεται μέσω του καναλιού `out` μπορεί να ληφθεί μέσω του καναλιού `in`. Αυτό μας επιτρέπει να επιστρέψουμε το κανάλι `out` στο οποίο μπορούμε να κάνουμε" κλήσεις μεθόδου "χωρίς να επιτρέψουμε σε άλλες διαδικασίες να παρεμποδίζουν αιτήματα που προορίζονται για τη διαδικασία μας.

Το νέο κανάλι `table` θα χρησιμοποιηθεί για τη δημιουργία καναλιών για εσωτερική χρήση.

5) Εδω διαβιβάσαμε επανειλημμένα μηνύματα με arity μεγαλύτερη ή ίση με δύο. Η μεταβλητή `method` συνδέεται με το πρώτο επιχείρημα· περιμένουμε μια συμβολοσειρά που ονομάζει τη μέθοδο, ακριβώς όπως και η διαδικασία του συστήματος. Η μεταβλητή `ack` δεσμεύεται σε ένα κανάλι στο οποίο θα στείλουμε οποιοδήποτε αποτέλεσμα της κλήσης μεθόδου. Η μεταβλητή `rest` συνδέεται με μια πλειάδα που περιέχει το υπόλοιπο μέρος των μηνυμάτων.

6) Η κατασκευή `match ... case` μας επιτρέπει να ταιριάξουμε το πρότυπο για τη δομή μιας διαδικασίας. Αυτό το χρησιμοποιούμεγια να κάνουμε αποστολή μηνυμάτων.

7-12) Εάν η `method` είναι η συμβολοσειρά `"new"`, τότε υποθέτουμε στη γραμμή 8 ότι η `rest` θα είναι μια πλειάδα με ένα στοιχείο, την αρχική τιμή. Δημιουργούμε ένα κανάλι `ticket` και το επιστρέφουμε χρησιμοποιώντας το κανάλι`ack`. Επίσης, συνδυάζουμε τις διαδικασίες `*ticket` με το `*table` σε μια νέα διαδικασία και στη συνέχεια παράγουμε ένα όνομα καναλιού από αυτά. Δεδομένου ότι έχουμε μόνο πρόσβαση στο `table`, μπορούμε να χειριστούμε μόνο τα δεδομένα που είναι αποθηκευμένα σε κανάλια που έχουν κατασκευαστεί με αυτόν τον τρόπο. Το κανάλι `ticket` συμπεριφέρεται σαν ένα εξαργυρώσιμο εισιτήριο για ένα ελεγχόμενο coat και το κανάλι `@(*ticket | *table)` είναι η καταχώρηση πίνακα που υποδεικνείεται από αυτό το κλειδί.
...
4) Σε αυτό το πλαίσιο, η `select` συμπεριφέρεται σαν `try` σε άλλες γλώσσες. Μόνο μία από τις παραλαβές στις γραμμές 5 και 10 θα προχωρήσει· αγωνίζονται για να δουν ποιος παίρνει πρώτα ένα μήνυμα. Εάν οι γραμμές 1-3 έχουν αμετάβλητο ότι είτε ένα αποτέλεσμα στέλνεται στην `ret` είτε ένα σφάλμα στην `err`, τότε δεν θα υπάρξει αγώνας. Εάν, από την άλλη πλευρά, θέλουμε να στείλουμε τόσο αποτέλεσμα όσο και σφάλμα, πρέπει να χρησιμοποιήσουμε την `for`:
...
## Φιλόσοφοι που διπνούν και αδιέξοδο
...
Αυτή η διαδικασία δεσμεύει μια μεταβλητή διαδικασίας `x` στο μήνυμα που ελήφθη στην `y`, αλλά επιμένει επίσης ότι η `x` είναι ένας ακέραιος αριθμός.

Υπάρχει ένα αμερικανικό αίνιγμα που λέει: "Πώς να κάνετε δεκαπέντε σέντς σε ψιλά όταν ένα νόμισμα δεν είναι τάληρο και το άλλο δεν είναι δεκάρα;" Ένα τάληρο αξίζει πέντε λεπτά και μια δεκάρα αξίζει δέκα. Ας υποθέσουμε ότι έχουμε τα ακόλουθα μηνύματα που αποστέλλονται στην `coins`, που κωδικοποιούν τα διαθέσιμα είδη αμερικανικών νομισμάτων αξίας μικρότερης των δεκαπέντε σέντς:
...
Ο χειριστής `!!` σημαίνει ότι τα μηνύματα πρέπει να παραμένουν στο κανάλι μόνιμα και να μην καταναλώνονται όταν λαμβάνονται από μια κατασκευή `for`.
   ...
   new x in { x!!("Γεια σας!") | for (msg <= x) { system!("print", msg) } }

Η παραπάνω διαδικασία θα εκτυπώσει "Γεια σας!" για όσο διάστημα λειτουργεί η εικονική μηχανή.
...
Αυτός ο όρος έχει δύο διαδικασίες που ακούν στο κανάλι `get` και ένα μόνο μήνυμα που στέλνεται στο` get`. Μόνο μία από τις δύο διαδικασίες θα είναι σε θέση να λάβει το μήνυμα.
...
Μπορούμε να εφαρμόσουμε την ανάκληση δημιουργώντας έναν προωθητή με έναν kill διακόπτη.
...
14-15) Εάν ένα μήνυμα έχει ποτέ σταλεί στο κανάλι `kill`, ορίσαμε το 'forwardFlag' σε false, το οποίο σταματά την προώθηση μηνυμάτων.
...
Ένας αποστολέας σύνδεσης μπορεί να καταγράψει όλα τα μηνύματα που έχουν σταλεί σε ένα κανάλι, στέλνοντας τα σε ένα δεύτερο κανάλι.
...
Ένα ζεύγος σφραγίσματος/αποσφραγίσματος παρέχει την ίδια λειτουργικότητα με τα δημόσια κλειδιά, αλλά χωρίς κρυπτογραφία. Είναι απλώς μια εξασθένηση του ελέγχου που περιγράφεται παραπάνω. Αυτό το σχέδιο μοτίβο μπορεί να χρησιμοποιηθεί για να υπογράψει κάτι για λογαριασμό του χρήστη. Στον οδηγό για το blockchain στη Rholang, θα δούμε ότι λειτουργεί ακόμη και στο blockchain επειδή δεν υπάρχουν μυστικά προς αποθήκευση, μόνο τα unforgeable ονόματα που να παραμείνουν μη προσβάσιμα.

### Προσοσχή στην αποστολή εξασθενητών

Μια βασική αρχή που πρέπει να θυμάστε με τις διαδικασίες RChain είναι αυτή που είναι παρόμοια με τις πιο παραδοσιακές εφαρμογές ιστού: οποιοσδήποτε κώδικας στέλνετε σε άλλο μέρος μπορεί να αποσυναρμολογηθεί. Από τα τέλη της δεκαετίας του 1990, όταν έγινε η αγορά των πραγμάτων μέσω του διαδικτύου, [υπήρξαν πλατφόρμες ηλεκτρονικού εμπορίου] (https://blog.detectify.com/2016/11/17/7-most-common-e-commerce- λάθη ασφαλείας /) όπου η πλατφόρμα βασίστηκε στα προγράμματα περιήγησης των χρηστών για να στείλει τη σωστή τιμή του στοιχείου πίσω σε αυτήν. Οι συγγραφείς δεν σκέφτονται ότι ο χρήστης ανοίγει τα εργαλεία για προγραμματιστές και αλλάζει την τιμή πριν να αποσταλεί. Ο σωστός τρόπος για να δημιουργήσετε μια πλατφόρμα ηλεκτρονικού εμπορίου είναι να αποθηκεύσετε τις τιμές στο διακομιστή και να τις ελέγξετε εκεί.

Ας υποθέσουμε ότι ο Bob είναι πρόθυμος να εκτελέσει κάποιο κώδικα για την Alice. έχει μια σύμβαση που λέει κάτι σαν "Πάρτε μια διαδικασία από αυτό το κανάλι και τρέξτε το".

     for (@P <- x) { P }

Αυτό είναι ακριβώς όπως ένα πρόγραμμα περιήγησης ιστού είναι πρόθυμος να εκτελέσει τον κώδικα JavaScript που παίρνει από έναν ιστότοπο. Αν η Αλίκη στείλει τον Bob σε έναν εξασθενητικό μεταφορέα, ο Μπομπ μπορεί να χρησιμοποιήσει το πρότυπο που ταιριάζει με τις παραγωγές στο Rholang για να ξεπεράσει τη διαδικασία και να αποκτήσει πρόσβαση στον υποκείμενο πόρο. Αντ 'αυτού, όπως στο παράδειγμα ηλεκτρονικού εμπορίου, η Alice πρέπει να στείλει μόνο κώδικα που διαβιβάζει τα αιτήματα στις δικές της διαδικασίες και κάνει την εξασθένηση εκεί.

## Συμπέρασμα

RChain είναι μια γλώσσα σχεδιασμένη για χρήση σε blockchain, αλλά δεν έχουμε αναφέρει τίποτα για τους κόμβους, τους χώρους ονομάτων, τα πορτοφόλια, τον Rev και phlogiston, τη δομή του δικτύου ή την Casper. Ένα προσεχές έγγραφο θα εξετάσει όλα αυτά τα θέματα και πολλά άλλα.

Ελπίζουμε ότι τα παραπάνω παραδείγματα προκαλούν την επιθυμία να γράψουμε περισσότερα κώδικα και να επιδείξουμε την ευκολία έκφρασης των ταυτόχρονων σχεδίων.
