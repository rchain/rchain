...
object RholangLinker {
  ...
  def splitOnPars(code: String): Vector[String] =
    code
      .foldLeft((List.empty[BlockDef], Vector(""))) {
          ...
          if (blockingList.isEmpty) {
            c match {
              ...
              case b if b == BlockDef.brace.open =>
                (
                  BlockDef.brace :: blockingList,
                  currResult.updated(currResult.length - 1, currResult.last + c)
                )
              case q if q == BlockDef.quote.open =>
                (
                  BlockDef.quote :: blockingList,
                  currResult.updated(currResult.length - 1, currResult.last + c)
                )
              case _ =>
                (blockingList, currResult.updated(currResult.length - 1, currResult.last + c))
            }
          } else {
  ...
  def findCloseBrace(startIndex: Int, str: String): Int = {
    val chars = str.iterator.zipWithIndex
      .dropWhile { case (_, i) => i <= startIndex }

    @tailrec
    def search(
        chars: Iterator[(Char, Int)],
        blockingList: List[BlockDef] = List(BlockDef.brace)
    ): Int = {
      val (c, i) = Try(chars.next()).getOrElse((' ', str.length))
      if (blockingList.isEmpty || i == str.length) {
        if (blockingList.nonEmpty) {
          throw new Exception("Imbalanced braces!")
        }
        i - 1
      } else {
        val blocker = blockingList.head
  ...
  def unpackImports(
      code: Code,
      unboundNames: Vector[String] = Vector.empty[String]
  ): (Code, Vector[String]) =
    code match {
  ...
  def processPackageDependencies(
      packages: Vector[NamedBlock]
  ): (Vector[NamedBlock], Map[String, Vector[String]]) = {
