...
trait ChargingReducer[F[_]] {
  ...
  def inj(par: Par)(implicit rand: Blake2b512Random, traceId: TraceId): F[Unit]

  def evalExpr(par: Par)(implicit env: Env[Par], traceId: TraceId): F[Par]

  def evalExprToPar(expr: Expr)(implicit env: Env[Par], traceId: TraceId): F[Par]
