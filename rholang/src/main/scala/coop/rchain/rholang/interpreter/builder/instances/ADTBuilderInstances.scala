...
trait ADTBuilderInstances {
  ...
  def checkTopLevel[F[_]: Sync, T](pv: F[ProcVisitOutputs[T]]): F[T] = pv.flatMap {
    normalizedTerm =>
  ...
  implicit def procSourceParInstance[F[_]: Sync]: ADTBuilder[F, Proc, SourcePar] =
    new ADTBuilder[F, Proc, SourcePar] {
      override def buildWithEnv(
          source: Proc,
          normalizerEnv: Map[String, SourcePar]
      ): F[SourcePar] =
        checkTopLevel(
          Normalizer[F, Proc, ProcVisitInputs, ProcVisitOutputs, SourcePar]
            .normalize(
              source,
              ProcVisitInputs(
                SourcePar(Position(0, 0, 0), VectorPar()),
                IndexMapChain.empty,
  ...
  implicit def stringSourceParInstance[F[_]: Sync]: ADTBuilder[F, String, SourcePar] =
    new ADTBuilder[F, String, SourcePar] {
      override def buildWithEnv(
          source: String,
          normalizerEnv: Map[String, SourcePar]
      ): F[SourcePar] =
        ASTBuilder[F, String, Proc]
          .build(source)
          .flatMap(p => ADTBuilder[F, Proc, SourcePar].buildWithEnv(p, normalizerEnv))
    }
