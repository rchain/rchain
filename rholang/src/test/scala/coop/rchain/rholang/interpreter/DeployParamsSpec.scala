...
class DeployParamsSpec extends fixture.FlatSpec with Matchers {
  override protected def withFixture(test: OneArgTest): Outcome = {
    ...
    val runtime   = Runtime.create[Id, Id](dbDir, size)
    runtime.reducer.setAvailablePhlos(Cost(Integer.MAX_VALUE))
  ...
  override type FixtureParam = Runtime[Id]
  ...
  "rho:deploy:params" should "return the parameters that are set." in { runtime =>
    ...
    shortLeashParams.setParams(empty, phloRate, empty, timestamp)
    runtime.reducer.eval(send)
