...
class CollectMatcherSpec extends FlatSpec with Matchers {
  ...
  "Set" should "delegate" in {
    ...
    val set = new PCollect(new CollectSet(setData, new RemainderVar(new ProcVarVar("Z"))))

    val result = ProcNormalizeMatcher.normalizeMatch[Coeval](set, inputs).value
    result.par should be(
      inputs.par.prepend(
        ParSet(
          Seq[Par](EPlus(EVar(BoundVar(1)), EVar(FreeVar(1))),
                   GInt(7),
                   GInt(8).prepend(EVar(FreeVar(2)))),
          connectiveUsed = true,
