...
class NameMatcherSpec extends FlatSpec with Matchers {
  ...
  "NameWildcard" should "add a wildcard count to knownFree" in {
    ...
    val result =
      Normalizer[Coeval, Name, NameVisitInputs, NameVisitOutputs, Par].normalize(nw, inputs).value
  ...
  "NameVar" should "Compile as FreeVar if it's not in env" in {
    val result =
      Normalizer[Coeval, Name, NameVisitInputs, NameVisitOutputs, Par].normalize(nvar, inputs).value
