...
trait PersistentStoreTester {
  def withTestSpace[R](errorLog: ErrorLog[Task])(f: TestFixture => R): R = {
    ...
    reducer.setAvailablePhlos(Cost(Integer.MAX_VALUE)).runSyncUnsafe(10.seconds)
    try {
...
class ReduceSpec extends FlatSpec with Matchers with PersistentStoreTester {
  ...
  "evalExpr" should "handle simple addition" in {
    ...
    errorLog.readAndClearErrorVector.runSyncUnsafe(10.seconds) should be(
      Vector.empty[InterpreterError]
    )
  }
  ...
  "Running out of phlogistons" should "stop the evaluation" in {
    ...
    val test = withTestSpace(errorLog) {
      case TestFixture(_, reducer) =>
        ...
        reducer.setAvailablePhlos(notEnoughPhlos).runSyncUnsafe(10.seconds)
        val splitRand = rand.splitByte(0)
        val receive =
          Receive(
            Seq(
    ...
    val result = test.attempt.runSyncUnsafe(10.seconds)
    assert(result === Left(OutOfPhlogistonsError))
    errorLog.readAndClearErrorVector().runSyncUnsafe(10.seconds) should be(Vector.empty)
  }
}
