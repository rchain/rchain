...
final case class TestFixture(space: RhoISpace[Id], reducer: ChargingReducer[Id])

trait PersistentStoreTester {
  def withTestSpace[R](errorLog: ErrorLog[Id])(f: TestFixture => R): R = {
    ...
    val reducer = RholangOnlyDispatcher.create[Id, Id](space)._2
...
class ReduceSpec extends FlatSpec with Matchers with PersistentStoreTester {
  ...
  "eval of Bundle" should "evaluate contents of bundle" in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        implicit val env = Env[Par]()
        reducer.eval(bundleSend)(env, splitRand)
        space.store.toMap
    }
  ...
  it should "throw an error if names are used against their polarity" in {
    ...
    val receiveResult = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        implicit val env = Env[Par]()
        reducer.eval(receive)
        space.store.toMap
    }
    receiveResult should be(mutable.HashMap.empty)
    ...
    val send =
      Send(Bundle(x, readFlag = true), Seq(Expr(GInt(7L))))
    ...
    sendResult should be(mutable.HashMap.empty)
    errorLog.readAndClearErrorVector should be(
  ...
  "eval of Send" should "place something in the tuplespace." in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        val send =
          Send(channel, List(GInt(7L), GInt(8L), GInt(9L)), persistent = false, BitSet())
        implicit val env = Env[Par]()
        reducer.eval(send)(env, splitRand)
        space.store.toMap
    }
  ...
  "eval of Send | Receive" should "meet in the tuplespace and proceed." in {
    ...
    val sendFirstResult = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(send)(env, splitRand0)
        reducer.eval(receive)(env, splitRand1)
        space.store.toMap
    }
  ...
  "eval of Send of Receive | Receive" should "meet in the tuplespace and proceed." in {
    ...
    val bothResult = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(Par(receives = Seq(receive), sends = Seq(send)))(env, baseRand)
        space.store.toMap
    }
  ...
  "Simple match" should "capture and add to the environment." in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(matchTerm)(env, splitRand)
        space.store.toMap
    }
  ...
  "eval of Send | Send | Receive join" should "meet in the tuplespace and proceed." in {
    ...
    val send1 =
      Send(GString("channel1"), List(GInt(7L), GInt(8L), GInt(9L)), persistent = false, BitSet())
    val send2 =
      Send(GString("channel2"), List(GInt(7L), GInt(8L), GInt(9L)), persistent = false, BitSet())
    val receive = Receive(
      Seq(
        ReceiveBind(
          Seq(EVar(FreeVar(0)), EVar(FreeVar(1)), EVar(FreeVar(2))),
    ...
    val sendFirstResult = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(send1)(env, splitRand0)
        reducer.eval(send2)(env, splitRand1)
        reducer.eval(receive)(env, splitRand2)
        space.store.toMap
    }
  ...
  "eval of nth method" should "pick out the nth item from a list" in {
    ...
    val indirectResult = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(nthCallEvalToSend)(env, splitRand)
        space.store.toMap
    }
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, _) =>
        ...
        val reducer = RholangOnlyDispatcher
          .create[Id, Id](space, Map("rho:test:foo" -> byteName(42)))
          ._2
        reducer.setAvailablePhlos(Cost(Integer.MAX_VALUE))
        implicit val env = Env[Par]()
        reducer.eval(newProc)(env, splitRand)
        space.store.toMap
    }
  ...
  "eval of nth method in send position" should "change what is sent" in {
    ...
    // format: off
    checkData(result)(channel, Seq(Send(GString("result"), List(GString("Success")), persistent = false, BitSet())), splitRand)
    // format: on
    errorLog.readAndClearErrorVector should be(Vector.empty[InterpreterError])
  }
  ...
  "eval of `toByteArray` method on any process" should "return that process serialized" in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(wrapWithSend(toByteArrayCall))(env, splitRand)
  ...
  it should "return an error when `toByteArray` is called with arguments" in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        implicit val env = Env[Par]()
        reducer.eval(toByteArrayWithArgumentsCall)
        space.store.toMap
    }
  ...
  "eval of `toUtf8Bytes`" should "transform string to UTF-8 byte array (not the rholang term)" in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(wrapWithSend(toUtf8BytesCall))(env, splitRand)
        space.store.toMap
    }
  ...
  it should "return an error when `toUtf8Bytes` is called with arguments" in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(toUtfBytesWithArgumentsCall)
        space.store.toMap
    }
    result should be(mutable.HashMap.empty)
    errorLog.readAndClearErrorVector should be(
  ...
  "variable references" should "be substituted before being used." in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(proc)(env, splitRandSrc)
        space.store.toMap
    }
  ...
  it should "reference a variable that comes from a match in tuplespace" in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        ...
        reducer.eval(proc)(env, baseRand)
        space.store.toMap
    }
  ...
  "Running out of phlogistons" should "stop the evaluation" in {
    ...
    val result = Try {
      withTestSpace(errorLog) {
        case TestFixture(_, reducer) =>
          ...
          reducer.setAvailablePhlos(notEnoughPhlos)
          val splitRand = rand.splitByte(0)
          val receive =
            Receive(
              Seq(
    ...
    assert(result === Failure(OutOfPhlogistonsError))
