...
class ReduceSpec extends FlatSpec with Matchers with PersistentStoreTester {
  ...
  "eval of Bundle" should "evaluate contents of bundle" in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        val bundleSend =
          Bundle(Send(GPrivateBuilder("channel"), List(GInt(7), GInt(8), GInt(9)), false, BitSet()))
        implicit val env = Env[Par]()
    ...
    val channel: Par = GPrivateBuilder("channel")
  ...
  "eval of Send" should "place something in the tuplespace." in {
    ...
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        val send =
          Send(GPrivateBuilder("channel"), List(GInt(7), GInt(8), GInt(9)), false, BitSet())
        implicit val env = Env[Par]()
  ...
  "eval of single channel Receive" should "place something in the tuplespace." in {
    ...
    val channels = List[Par](GPrivateBuilder("channel"))
  ...
  "eval of Send | Receive" should "meet in the tuplespace and proceed." in {
    ...
    val receive = Receive(
      ...
      Send(GPrivateBuilder("result"), List(GString("Success")), false, BitSet()),
      false,
      3,
      BitSet()
    )
    ...
    val channel: Par = GPrivateBuilder("result")
  ...
  "eval of Send | Receive" should "when whole list is bound to list remainder, meet in the tuplespace and proceed. (RHOL-422)" in {
    ...
    // format: off
    val send =
      Send(GPrivateBuilder("channel"), List(Par(exprs = Seq(Expr(EListBody(EList(Seq(GInt(7), GInt(8), GInt(9)))))))), false, BitSet())
    val receive = Receive(
      Seq(
  ...
  "eval of Send of Receive | Receive" should "meet in the tuplespace and proceed." in {
    ...
    val simpleReceive = Receive(
      Seq(ReceiveBind(Seq(GInt(2)), GPrivateBuilder("2"))),
      Par(),
      false,
      0,
      BitSet()
    ...
    val send =
      Send(GPrivateBuilder("1"), Seq[Par](simpleReceive), false, BitSet())
    val receive = Receive(
      Seq(ReceiveBind(Seq(EVar(FreeVar(0))), GPrivateBuilder("1"), freeCount = 1)),
      EVar(BoundVar(0)),
      false,
      1,
      BitSet()
    ...
    val channels = List[Par](GPrivateBuilder("2"))
  ...
  "eval of Send | Send | Receive join" should "meet in the tuplespace and proceed." in {
    ...
    val send1 =
      Send(GPrivateBuilder("channel1"), List(GInt(7), GInt(8), GInt(9)), false, BitSet())
    val send2 =
      Send(GPrivateBuilder("channel2"), List(GInt(7), GInt(8), GInt(9)), false, BitSet())
    val receive = Receive(
      Seq(
        ReceiveBind(
          Seq(EVar(FreeVar(0)), EVar(FreeVar(1)), EVar(FreeVar(2))),
          GPrivateBuilder("channel1"),
          freeCount = 3
        ),
        ReceiveBind(
          Seq(EVar(FreeVar(0)), EVar(FreeVar(1)), EVar(FreeVar(2))),
          GPrivateBuilder("channel2"),
          freeCount = 3
        )
      ),
      Send(GPrivateBuilder("result"), List(GString("Success")), false, BitSet()),
  ...
  "eval of Send with remainder receive" should "capture the remainder." in {
    ...
    val receive =
      Receive(
        Seq(ReceiveBind(Seq(), GPrivateBuilder("channel"), Some(FreeVar(0)), freeCount = 1)),
        Send(GPrivateBuilder("result"), Seq(EVar(BoundVar(0))))
      )
  ...
  "eval of New" should "use deterministic names and provide urn-based resources" in {
    ...
    val newProc: New =
      New(
        ...
        p = Par(
          sends = List(
            Send(GPrivateBuilder("result0"), List(EVar(BoundVar(0))), locallyFree = BitSet(0)),
            Send(GPrivateBuilder("result1"), List(EVar(BoundVar(1))), locallyFree = BitSet(1))
          ),
    ...
    val channel0: Par = GPrivateBuilder("result0")
    val channel1: Par = GPrivateBuilder("result1")
    // format: off
    result should be(
      HashMap(
        List(channel0) ->
  ...
  "eval of nth method in send position" should "change what is sent" in {
    ...
    val send: Par =
      Send(GPrivateBuilder("result"), List[Par](nthCallEvalToSend), false, BitSet())
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        implicit val env = Env[Par]()
            ...
            List(
              Datum.create(
                ...
                ListParWithRandom(Seq(
                  Send(GPrivateBuilder("result"), List(GString("Success")), false, BitSet())), splitRand),
                false)),
            List())
      )
    )
  ...
  "eval of `toByteArray` method on any process" should "return that process serialized" in {
    ...
    def wrapWithSend(p: Par): Par = Send(GPrivateBuilder("result"), List[Par](p), false, BitSet())
    val result = withTestSpace(errorLog) {
      case TestFixture(space, reducer) =>
        val env         = Env[Par]()
  ...
  it should "return an error when `toByteArray` is called with arguments" in {
    ...
    val toByteArrayWithArgumentsCall: EMethod =
      EMethod(
        "toByteArray",
        Par(
          sends = Seq(Send(GPrivateBuilder("result"), List(GString("Success")), false, BitSet()))
        ),
        List[Par](GInt(1))
      )
      ...
      p = Par(
        ...
        receives = List(
          Receive(
            ...
            body = Send(chan = GPrivateBuilder("result"), data = List(GString("true"))),
            bindCount = 0
          )
        )
      )
      ...
      p = Match(
        ...
        cases = List(
          MatchCase(
            ...
            source = Send(chan = GPrivateBuilder("result"), data = List(GString("true")))
          )
        )
      )
    )
  ...
  it should "reference a variable that comes from a match in tuplespace" in {
    ...
    val mergeRand         = Blake2b512Random.merge(Seq(splitRand1, splitRand0))
    val proc = Par(
      sends = List(Send(chan = GPrivateBuilder("7"), data = List(GInt(10)))),
      receives = List(
        Receive(
          binds = List(
            ReceiveBind(
              patterns = List(EVar(FreeVar(0))),
              source = GPrivateBuilder("7"),
              freeCount = 1
            )
          ),
          body = Match(
